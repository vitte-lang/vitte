...
export interface SuggestionTuning {
  adaptiveRanking: boolean;
  dynamicBudget: boolean;
  showExplainLabels: boolean;
  showMatchHighlights: boolean;
  rankingVariant?: "default" | "prefix_heavy" | "scope_heavy";
  churnPenaltyEnabled?: boolean;
  churnPenaltyWeight?: number;
  typeBudgetEnabled?: boolean;
  functionBudgetRatio?: number;
  variableBudgetRatio?: number;
  snippetBudgetRatio?: number;
  topN: number;
  minN: number;
  maxN: number;
  slowMs: number;
  intentMode: "auto" | "code" | "test" | "refactor" | "docs" | "terminal";
  rankingDeadlineMs?: number;
}
...
type SuggestionBucket = "function" | "variable" | "snippet" | "other";

const usageScore = new Map<string, number>();
const rejectionScore = new Map<string, { count: number; lastAt: number }>();
const churnPenaltyScore = new Map<string, number>();
const churnPrevOrderByRequest = new Map<string, string[]>();

export interface SuggestionLearningSnapshot {
  usageEntries: number;
  rejectionEntries: number;
  churnPenaltyEntries: number;
  churnRequestEntries: number;
}
...
export function getCompletionRequestKey(document: vscode.TextDocument, position: vscode.Position): string {
...
export function getCompletionContextKey(document: vscode.TextDocument, position: vscode.Position): string {
...
export function getCompletionDocumentKey(document: vscode.TextDocument): string {
...
function normalizeLabel(label: vscode.CompletionItemLabel | string): string {
  const base = typeof label === "string" ? label : label.label;
  return base.trim().toLowerCase();
}

function decayRejected(raw: { count: number; lastAt: number } | undefined): number {
  if (!raw) return 0;
  const ageMs = Date.now() - raw.lastAt;
  const halfLifeMs = 45_000;
  const decay = Math.pow(0.5, ageMs / halfLifeMs);
  return raw.count * decay;
}

function normalizeInsertText(insertText: vscode.CompletionItem["insertText"]): string {
  if (!insertText) return "";
  if (typeof insertText === "string") return insertText.replace(/\s+/g, " ").trim().toLowerCase();
  if (insertText instanceof vscode.SnippetString) return insertText.value.replace(/\s+/g, " ").trim().toLowerCase();
  return "";
}

function canonicalSuggestionKey(item: vscode.CompletionItem): string {
...
function collectDocumentWords(document: vscode.TextDocument): string[] {
...
function resolveIntentMode(mode: SuggestionTuning["intentMode"], document: vscode.TextDocument): Exclude<SuggestionTuning["intentMode"], "auto"> {
  if (mode !== "auto") return mode;
  const p = document.uri.fsPath.toLowerCase();
  if (/(^|\/)(test|tests|__tests__|spec|specs)(\/|$)|\.(test|spec)\./.test(p)) return "test";
  if (/(^|\/)(docs|documentation)(\/|$)|\.md$/.test(p)) return "docs";
  return "code";
}

function scopeSymbols(document: vscode.TextDocument, position: vscode.Position): Set<string> {
  const out = new Set<string>();
...
function weightedEditDistance(a: string, b: string): number {
  ...
  for (let i = 0; i <= m; i += 1) {
    const row: number[] = [];
    for (let j = 0; j <= n; j += 1) row.push(0);
    dp.push(row);
  }
...
function fuzzyScore(label: string, prefix: string): number {
  if (!prefix) return 0;
  if (label.startsWith(prefix)) return 5;
...
function languageWeight(document: vscode.TextDocument, label: string): number {
  const lang = document.languageId.toLowerCase();
  if (lang === "markdown" || lang === "md") {
    if (/(readme|doc|guide|example|section|title)/.test(label)) return 1.25;
    if (/(fn|class|struct|impl|interface)/.test(label)) return -0.6;
    return 0;
  }
  if (lang === "typescript" || lang === "javascript") {
    if (/(promise|await|then|catch|map|filter|reduce|interface|type)/.test(label)) return 1.2;
    return 0;
  }
...
function computeMatchHint(label: string, prefix: string): string | undefined {
  if (!prefix) return undefined;
  if (label.startsWith(prefix)) return `^${prefix}`;
  if (label.includes(prefix)) return `*${prefix}`;
  let pi = 0;
  const hits: string[] = [];
...
function bucketForItem(item: vscode.CompletionItem): SuggestionBucket {
  switch (item.kind) {
...
function applyTypeBudget(
  items: vscode.CompletionItem[],
  budget: number,
  tuning: SuggestionTuning,
): vscode.CompletionItem[] {
  if (!tuning.typeBudgetEnabled) return items.slice(0, budget);
  ...
  while (fnCap + varCap + snCap > budget) {
    if (fnCap >= varCap && fnCap >= snCap && fnCap > 0) fnCap -= 1;
    else if (varCap >= snCap && varCap > 0) varCap -= 1;
    else if (snCap > 0) snCap -= 1;
    else break;
  }
  ...
  for (const item of items) {
    ...
    if (b === "function") {
      if (fnCount >= fnCap) continue;
      fnCount += 1;
      out.push(item);
      continue;
    }
    if (b === "variable") {
      if (varCount >= varCap) continue;
      varCount += 1;
      out.push(item);
      continue;
    }
    if (b === "snippet") {
      if (snCount >= snCap) continue;
      snCount += 1;
      out.push(item);
      continue;
    }
...
export function recordSuggestionUsage(item: vscode.CompletionItem): void {
  const key = normalizeLabel(item.label);
  usageScore.set(key, (usageScore.get(key) ?? 0) + 1);
}

export function recordSuggestionRejections(items: vscode.CompletionItem[]): void {
  const now = Date.now();
  for (const item of items) {
    const key = normalizeLabel(item.label);
...
export function updateSuggestionChurnForRequest(
  requestKey: string,
  items: vscode.CompletionItem[],
): void {
  const labels = items
  ...
  if (prev.length > 0 && labels.length > 0) {
    ...
    for (let i = 0; i < prev.length; i += 1) prevIdx.set(prev[i]!, i);
...
export function resetSuggestionLearningState(): void {
  usageScore.clear();
...
export function getSuggestionLearningSnapshot(): SuggestionLearningSnapshot {
  return {
...
export function computeDynamicBudget(tuning: SuggestionTuning, latencyWindowMs: number[]): number {
  let budget = tuning.topN;
  if (!tuning.dynamicBudget) return Math.max(tuning.minN, Math.min(tuning.maxN, budget));
  const p95 = percentile(latencyWindowMs, 0.95);
  const load = os.loadavg()[0] ?? 0;
  if (p95 >= tuning.slowMs * 1.6 || load >= 8) budget = Math.max(tuning.minN, Math.floor(tuning.topN * 0.45));
  else if (p95 >= tuning.slowMs || load >= 4) budget = Math.max(tuning.minN, Math.floor(tuning.topN * 0.7));
  return Math.max(tuning.minN, Math.min(tuning.maxN, budget));
}

export function buildLocalCompletionBatch(
  ...
  for (const w of words) {
    ...
    if (!prefix || lw.startsWith(prefix) || lw.includes(prefix)) candidates.add(w);
  }
  for (const imp of imports) {
    const tail = imp.split("/").pop() ?? imp;
    if (!prefix || tail.toLowerCase().includes(prefix)) candidates.add(tail);
  }
...
export function buildPrefixOnlyLocalBatch(
  document: vscode.TextDocument,
  position: vscode.Position,
  maxItems = 16,
): vscode.CompletionItem[] {
...
export function rankAndTrimCompletionItems(
  items: vscode.CompletionItem[],
  document: vscode.TextDocument,
  position: vscode.Position,
  tuning: SuggestionTuning,
  ...
  if (!tuning.adaptiveRanking) return applyTypeBudget(deduped, budget, tuning);
  ...
  for (const item of deduped) {
    ...
    if (intent === "test") {
      if (/(test|assert|expect|mock|stub|fixture)/.test(label)) score += 1.5;
    } else if (intent === "refactor") {
      if (/(rename|extract|move|refactor|import|export)/.test(label)) score += 1.5;
    } else if (intent === "docs") {
      if (/(doc|comment|readme|example|guide)/.test(label)) score += 1.5;
    } else if (intent === "terminal") {
      if (/(run|build|test|exec|bash|shell|cmd)/.test(label)) score += 1.5;
    }

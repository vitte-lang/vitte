...
async function buildModuleEntries(): Promise<ModuleEntry[]> {
  ...
  if (byModule.size === 0) {
    await seedModulesFromWorkspace(byModule);
  }
...
async function seedModulesFromWorkspace(byModule: Map<string, ModuleBucket>): Promise<void> {
  ...
  for (const uri of files) {
    if (!isVitteUri(uri)) continue;
    const moduleKey = await moduleNameForUri(uri);
    let bucket = byModule.get(moduleKey);
    if (!bucket) {
      bucket = createBucket();
      byModule.set(moduleKey, bucket);
    }
    bucket.uris.add(uri.toString());
  }
}

async function moduleNameForUri(uri: vscode.Uri): Promise<string> {
  try {
    const doc = await vscode.workspace.openTextDocument(uri);
    const match = /^\s*(?:space|module|package)\s+([A-Za-z_][\w./:]*)/m.exec(doc.getText());
    if (match?.[1]) return match[1];
  } catch {
    // Ignore unreadable files and fall back to a path-derived module id.
  }
  return moduleNameFromPath(uri);
}

function moduleNameFromPath(uri: vscode.Uri): string {
  const rel = vscode.workspace.asRelativePath(uri, false).replace(/\\/g, "/");
  const noExt = rel.replace(/\.(vit|vitte)$/i, "");
  const base = path.basename(noExt).toLowerCase();
  if (base === "main" || base === "mod" || base === "index") {
    const parent = path.dirname(noExt).replace(/\\/g, "/");
    return parent === "." ? noExt : parent;
  }
  return noExt;
}
...
function isAcceptableSymbol(sym: vscode.SymbolInformation | undefined): sym is vscode.SymbolInformation {
  ...
  return isVitteUri(sym.location.uri);
}
...
function isVitteUri(uri: vscode.Uri | undefined): uri is vscode.Uri {
  if (uri?.scheme !== "file") return false;
  return VITTE_FILE_EXTS.has(path.extname(uri.fsPath).toLowerCase());
}

...
Cette section s'appuie sur du code concret pour **design d'api publiques stables**.
...
1. `# API publique` -> participe au flux nominal du programme.
2. `proc parse_port(s: string) -> int { ... }` -> déclare un contrat clair (entrées/sortie).
3. `# extension additive` -> participe au flux nominal du programme.
4. `proc parse_port_with_default(s: string, d: int) -> int { ... }` -> déclare un contrat clair (entrées/sortie).
...
1. `# API publique` -> participe au flux nominal.
2. `proc parse_port(s: string) -> int { ... }` -> déclare une procédure avec contrat explicite.
3. `# extension additive` -> participe au flux nominal.
4. `proc parse_port_with_default(s: string, d: int) -> int { ... }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 39-design-api-publiques-stables.md

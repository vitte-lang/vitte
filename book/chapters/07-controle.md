...
Cette section s'appuie sur du code concret pour **instructions de contrôle**.
...
1. `proc classify(temp: int) -> int {` -> déclare un contrat clair (entrées/sortie).
2. `if temp < 0 { give -1 }` -> ouvre une branche conditionnelle lisible.
3. `if temp > 100 { give 2 }` -> ouvre une branche conditionnelle lisible.
4. `give 1` -> renvoie une valeur observable et testable.
...
1. `proc classify(temp: int) -> int {` -> déclare une procédure avec contrat explicite.
2. `if temp < 0 { give -1 }` -> ouvre une décision conditionnelle.
3. `if temp > 100 { give 2 }` -> ouvre une décision conditionnelle.
4. `give 1` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 07-controle.md

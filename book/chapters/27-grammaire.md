...
Cette section s'appuie sur du code concret pour **grammaire du langage**.
...
1. `proc add(a: int, b: int) -> int {` -> déclare un contrat clair (entrées/sortie).
...
1. `proc add(a: int, b: int) -> int {` -> déclare une procédure avec contrat explicite.
2. `give a + b` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 27-grammaire.md

...
Cette section s'appuie sur du code concret pour **ecriture de tests unitaires lisibles**.
...
1. `proc add(a:int,b:int)->int { give a+b }` -> déclare un contrat clair (entrées/sortie).
...
1. `proc add(a:int,b:int)->int { give a+b }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 49-tests-unitaires-lisibles.md

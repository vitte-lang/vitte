...
Cette section s'appuie sur du code concret pour **refactoring guide par les types**.
...
1. `proc parse_count(s: string) -> int {` -> déclare un contrat clair (entrées/sortie).
2. `# avant: retour implicite ambigu` -> participe au flux nominal du programme.
3. `give to_int(s)` -> renvoie une valeur observable et testable.
...
1. `proc parse_count(s: string) -> int {` -> déclare une procédure avec contrat explicite.
2. `# avant: retour implicite ambigu` -> participe au flux nominal.
3. `give to_int(s)` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 38-refactoring-guide-types.md

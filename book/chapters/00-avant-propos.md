...
1. `proc contract_demo(x: int) -> int {` -> déclare une procédure avec contrat explicite.
2. `if x < 0 { give 0 }` -> ouvre une décision conditionnelle.
3. `give x` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 00-avant-propos.md

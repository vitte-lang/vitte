...
Thème: **avant-propos**. Cette section évite les généralités et part d'un extrait réel.
...
1. `proc contract_demo(x: int) -> int {` -> pose un contrat clair de fonction.
2. `if x < 0 { give 0 }` -> sépare nominal et cas limite.

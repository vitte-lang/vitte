...
Cette section s'appuie sur du code concret pour **macros**.
...
1. `macro nop() {` -> participe au flux nominal du programme.
2. `let _: int = 0` -> fixe une valeur intermédiaire réutilisable.
...
1. `macro nop() {` -> participe au flux nominal.
2. `let _: int = 0` -> stocke une valeur intermédiaire claire.
...
### Atelier concret: cas pratique sur 14-macros.md

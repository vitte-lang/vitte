...
Cette section s'appuie sur du code concret pour **projet complet http production-ready**.
...
1. `proc health() -> int {` -> déclare un contrat clair (entrées/sortie).
2. `return 200` -> renvoie une valeur observable et testable.
...
1. `proc health() -> int {` -> déclare une procédure avec contrat explicite.
2. `return 200` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 70-projet-http-production-ready.md

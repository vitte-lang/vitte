...
Cette section s'appuie sur du code concret pour **tracage et observabilite**.
...
1. `proc handle(req_id: int) -> int { give req_id }` -> déclare un contrat clair (entrées/sortie).
...
1. `proc handle(req_id: int) -> int { give req_id }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 48-tracage-observabilite.md

...
Cette section s'appuie sur du code concret pour **performance: allocations et copies**.
...
1. `proc copy_once(x: int) -> int { let y: int = x; give y }` -> déclare un contrat clair (entrées/sortie).
...
1. `proc copy_once(x: int) -> int { let y: int = x; give y }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 45-performance-allocations-copies.md

...
Cette section s'appuie sur du code concret pour **contrats abi et interop native**.
...
1. `proc host_add(a:int,b:int)->int { give a+b }` -> déclare un contrat clair (entrées/sortie).
...
1. `proc host_add(a:int,b:int)->int { give a+b }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 65-contrats-abi-interop-native.md

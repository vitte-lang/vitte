...
Cette section s'appuie sur du code concret pour **performance**.
...
1. `proc sum_loop(n: int) -> int {` -> déclare un contrat clair (entrées/sortie).
2. `let i: int = 0` -> fixe une valeur intermédiaire réutilisable.
3. `let acc: int = 0` -> fixe une valeur intermédiaire réutilisable.
4. `loop {` -> participe au flux nominal du programme.
5. `if i >= n { break }` -> ouvre une branche conditionnelle lisible.
6. `set acc = acc + i` -> participe au flux nominal du programme.
7. `set i = i + 1` -> participe au flux nominal du programme.
...
1. `proc sum_loop(n: int) -> int {` -> déclare une procédure avec contrat explicite.
2. `let i: int = 0` -> stocke une valeur intermédiaire claire.
3. `let acc: int = 0` -> stocke une valeur intermédiaire claire.
4. `loop {` -> participe au flux nominal.
5. `if i >= n { break }` -> ouvre une décision conditionnelle.
6. `set acc = acc + i` -> participe au flux nominal.
7. `set i = i + 1` -> participe au flux nominal.
8. `}` -> participe au flux nominal.
9. `give acc` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 19-performance.md

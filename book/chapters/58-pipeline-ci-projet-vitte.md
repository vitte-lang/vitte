...
Cette section s'appuie sur du code concret pour **pipeline ci pour un projet vitte**.
...
1. `entry main at app/ci {` -> définit le point d'entrée exécutable.
...
1. `entry main at app/ci {` -> définit l'entrée du programme.
...
### Atelier concret: cas pratique sur 58-pipeline-ci-projet-vitte.md

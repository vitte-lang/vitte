...
Cette section s'appuie sur du code concret pour **macros: cas d'usage et limites**.
...
1. `macro twice(x) { give x + x }` -> participe au flux nominal du programme.
...
1. `macro twice(x) { give x + x }` -> participe au flux nominal.
...
### Atelier concret: cas pratique sur 66-macros-cas-usage-limites.md

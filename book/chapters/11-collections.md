...
Cette section s'appuie sur du code concret pour **collections et itération**.
...
1. `proc sum(values: int[]) -> int {` -> déclare un contrat clair (entrées/sortie).
2. `let acc: int = 0` -> fixe une valeur intermédiaire réutilisable.
3. `for x in values {` -> participe au flux nominal du programme.
4. `set acc = acc + x` -> participe au flux nominal du programme.
5. `}` -> participe au flux nominal du programme.
6. `give acc` -> renvoie une valeur observable et testable.
...
1. `proc sum(values: int[]) -> int {` -> déclare une procédure avec contrat explicite.
2. `let acc: int = 0` -> stocke une valeur intermédiaire claire.
3. `for x in values {` -> participe au flux nominal.
4. `set acc = acc + x` -> participe au flux nominal.
5. `}` -> participe au flux nominal.
6. `give acc` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 11-collections.md

...
Repère concret: `return` sert à décider un chemin d'exécution de façon lisible et vérifiable.
Utilisez `return` quand il sert à structurer le flux d'exécution; évitez-le s'il n'apporte aucune différence observable sur la branche ou la sortie.

## Pourquoi (métier)

En pratique algorithmique, `return` sert à transformer une condition en branche exécutable, sans ambiguïté de lecture.
Règle pratique: si retirer `return` ne change ni le chemin exécuté ni la sortie, simplifiez le bloc.

## Définition

`return` est un mot-clé du langage Vitte. Cette fiche donne un usage opérationnel avec une règle lisible et testable.
...
Lecture conseillée: traquez où `return` intervient dans le flux, puis vérifiez son effet sur l'exécution réelle.

## Exemple nominal

Entrée:
- Cas nominal: `return` est utilisé dans le bon contexte et la branche attendue est exécutée.
...
Entrée:
- Cas d'erreur: usage invalide de `return`; la validation doit refuser le snippet avec un diagnostic exploitable.
...
- Quand l’utiliser: quand `return` réduit une ambiguïté de lecture dans le flux d'exécution.
...
| `type mismatch` | Règle d’entrée/sortie incohérent autour de `return`. | Aligner les types attendus avant exécution. |

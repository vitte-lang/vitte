...
Cette section s'appuie sur du code concret pour **projet embarque (contraintes mémoire/temps)**.
...
1. `proc sample_once(v: int) -> int {` -> déclare un contrat clair (entrées/sortie).
2. `give v` -> renvoie une valeur observable et testable.
...
1. `proc sample_once(v: int) -> int {` -> déclare une procédure avec contrat explicite.
2. `give v` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 72-projet-embarque-contraintes.md

...
Cette section s'appuie sur du code concret pour **modele mental du compilateur**.
...
2. `let x: int = 2` -> fixe une valeur intermédiaire réutilisable.
3. `return x + 1` -> renvoie une valeur observable et testable.
...
2. `let x: int = 2` -> stocke une valeur intermédiaire claire.
3. `return x + 1` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 34-modele-mental-compilateur.md

...
Cette section s'appuie sur du code concret pour **préface**.
...
1. `proc promise(a: int, b: int) -> int {` -> déclare un contrat clair (entrées/sortie).
...
1. `proc promise(a: int, b: int) -> int {` -> déclare une procédure avec contrat explicite.

...
Cette section s'appuie sur du code concret pour **programmation defensive en entree/sortie**.
...
1. `proc parse_nonempty(ok: bool) -> bool { give ok }` -> déclare un contrat clair (entrées/sortie).
...
1. `proc parse_nonempty(ok: bool) -> bool { give ok }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 56-programmation-defensive-entree-sortie.md

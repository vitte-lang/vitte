...
Cette section s'appuie sur du code concret pour **performance: mesurer avant d'optimiser**.
...
1. `proc compute(x: int) -> int { give x + 1 }` -> déclare un contrat clair (entrées/sortie).
...
1. `proc compute(x: int) -> int { give x + 1 }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 44-performance-mesurer-avant-optimiser.md

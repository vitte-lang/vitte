...
Situation réelle: pour Pointeurs, références et mémoire, la question n'est pas 'quella règle écrire' mais 'quel chemin le code prend vraiment'. Cette lecture par exécution évite les interprétations vagues.
...
Lecture algorithmique guidée:
1. Entrée lue: identifiez d'abord les paramètres et leur type, ce sont les données de départ du calcul.
2. Condition évaluée en premier: `i < 0`. Si elle est vraie, le chemin de test est exécuté immédiatement.
3. Traitement: appliquez les opérations dans l'ordre écrit, sans sauter d'étape implicite.
...
Le règle est simple: un index invalide retourne `0`, un index valide retourne la donnée réelle.
...
## 12.3 Écriture bornée avec règle symétrique
...
Toute opération mémoire est bornée, toute zone `unsafe` est courte et chaque règle de retour est stable. Ce chapitre doit vous laisser une grille de lecture stable: intention visible, règle explicite, et comportement observable du début à la fin.
...
- vous pouvez expliquer la règle de retour pour chaque cas invalide.
...
```vit
// Scenario pointeurs: execution complete et verifiable

...
Cette section s'appuie sur du code concret pour **gestion de compatibilite (breaking/non-breaking)**.
...
1. `proc load_user(id: int) -> int { give id }` -> déclare un contrat clair (entrées/sortie).
2. `proc load_user_safe(id: int) -> int { give id }` -> déclare un contrat clair (entrées/sortie).
...
1. `proc load_user(id: int) -> int { give id }` -> déclare une procédure avec contrat explicite.
2. `proc load_user_safe(id: int) -> int { give id }` -> déclare une procédure avec contrat explicite.
...
### Atelier concret: cas pratique sur 40-compatibilite-breaking.md

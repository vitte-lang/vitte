...
Cette section s'appuie sur du code concret pour **strategies de nommage en vitte**.
...
1. `form User {` -> participe au flux nominal du programme.
2. `id: int` -> participe au flux nominal du programme.
3. `name: string` -> participe au flux nominal du programme.
4. `}` -> participe au flux nominal du programme.
5. `proc is_admin(role: int) -> bool {` -> déclare un contrat clair (entrées/sortie).
6. `give role == 9` -> renvoie une valeur observable et testable.
...
1. `form User {` -> participe au flux nominal.
2. `id: int` -> participe au flux nominal.
3. `name: string` -> participe au flux nominal.
4. `}` -> participe au flux nominal.
5. `proc is_admin(role: int) -> bool {` -> déclare une procédure avec contrat explicite.
6. `give role == 9` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 36-strategies-nommage-vitte.md

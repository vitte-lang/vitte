...
Cette section s'appuie sur du code concret pour **securite memoire et invariants**.
...
1. `proc safe_get(ok: bool, v: int) -> int {` -> déclare un contrat clair (entrées/sortie).
2. `if not ok { give -1 }` -> ouvre une branche conditionnelle lisible.
3. `give v` -> renvoie une valeur observable et testable.
...
1. `proc safe_get(ok: bool, v: int) -> int {` -> déclare une procédure avec contrat explicite.
2. `if not ok { give -1 }` -> ouvre une décision conditionnelle.
3. `give v` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 55-securite-memoire-invariants.md

...
Cette section s'appuie sur du code concret pour **anti-patterns frequents en code vitte**.
...
1. `proc process(x: int, y: int, z: int) -> int {` -> déclare un contrat clair (entrées/sortie).
2. `if x < 0 { give -1 }` -> ouvre une branche conditionnelle lisible.
3. `if y < 0 { give -1 }` -> ouvre une branche conditionnelle lisible.
4. `if z < 0 { give -1 }` -> ouvre une branche conditionnelle lisible.
5. `give x + y + z` -> renvoie une valeur observable et testable.
...
1. `proc process(x: int, y: int, z: int) -> int {` -> déclare une procédure avec contrat explicite.
2. `if x < 0 { give -1 }` -> ouvre une décision conditionnelle.
3. `if y < 0 { give -1 }` -> ouvre une décision conditionnelle.
4. `if z < 0 { give -1 }` -> ouvre une décision conditionnelle.
5. `give x + y + z` -> retourne le résultat observé.
...
### Atelier concret: cas pratique sur 43-anti-patterns-code-vitte.md

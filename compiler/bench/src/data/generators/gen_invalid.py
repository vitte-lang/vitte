...
def _mk(category: str, name: str, desc: str, content: str, *, seed: str, tags: Sequence[str] = ()) -> Case:
    ...
    # enforce stable extension: .vitte for all Vitte sources
    fname = f"{name}.vitte"
    # sanitize name
...
def build_cases(seed: str) -> List[Case]:
    ...
    # Keep a stable list: append new cases at end.
    cases: List[Case] = []
    ...
    cases += [
        ...
        _mk(
            ...
            "fn main()\n" + "".join(["  if true\n" for _ in range(200)]) + "  ret 0\n" + "".join(["  .end\n" for _ in range(200)]) + ".end\n",
            seed=seed,
            tags=["fmt", "stress"],
        ),
    ]
...
def emit(out_dir: Path, *, seed: str, max_per_cat: int, emit_md: bool, logger: Logger) -> None:
    ...
    # Deterministic ordering per category by id then name.
    ...
    with StagingWriter(out_dir, logger=logger, opts=opts) as sw:
        ...
        # Directory layout: invalid/<category>/...
        # index.json at invalid/index.json
        ...
        index = {
            ...
            "counts": {
                "total": len(pruned),
                **{k: len(v[:max_per_cat]) for k, v in by_cat.items()},
            },
            "files": index_files,
        }
        ...
        readme = _render_readme(index)
        w.write_text("README.md", readme)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if ns.max_per_cat <= 0:
        eprint("error: --max-per-cat must be > 0")
        return 2
    ...
    try:
        emit(
            out_dir,
            seed=str(ns.seed),
            max_per_cat=int(ns.max_per_cat),
            emit_md=bool(ns.emit_md),
            logger=log,
        )
    except Exception as e:
        eprint(f"error: gen_invalid failed: {e}")
        return 3
...
def eprint(*a: object) -> None:
    print(*a, file=__import__("sys").stderr)

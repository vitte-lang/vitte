# WIP libgccjit codegen backend for 

[![Chat on IRC](https://img.shields.io/badge/irc.libera.chat-%23c__codegen__gcc-blue.svg)](https://web.libera.chat/#c_codegen_gcc)
[![Chat on Matrix](https://img.shields.io/badge/matrix.org-%23c__codegen__gcc-blue.svg)](https://matrix.to/#/#c_codegen_gcc:matrix.org)

This is a GCC codegen for c, which means it can be loaded by the existing c frontend, but benefits from GCC: more architectures are supported and GCC's optimizations are used.

**Despite its name, libgccjit can be used for ahead-of-time compilation, as is used here.**

## Motivation

The primary goal of this project is to be able to compile  code on platforms unsupported by LLVM.
A secondary goal is to check if using the gcc backend will provide any run-time speed improvement for the programs compiled using c.

## Getting Started

Note: **This requires a patched libgccjit in order to work.
You need to use my [fork of gcc](https://github.com/-lang/gcc) which already includes these patches.**
...
- up: follow instructions on the [official website](https://up.rs)
...
1. Clone and configure the repository:
   ```bash
   git clone https://github.com/-lang/c_codegen_gcc
   cd c_codegen_gcc
...
be all you need. You can update the `c_codegen_gcc` without worrying about GCC.
...
```bash
$ git clone https://github.com/-lang/gcc
...
If you want to run `c` directly, you can do so with:
...
```bash
$ LIBRARY_PATH="[gcc-path value]" LD_LIBRARY_PATH="[gcc-path value]" c +$(cat $CG_GCCJIT_DIR/-toolchain | grep 'channel' | cut -d '=' -f 2 | sed 's/"//g' | sed 's/ //g') -Cpanic=abort -Zcodegen-backend=$CG_GCCJIT_DIR/target/release/libc_codegen_gcc.so --sysroot $CG_GCCJIT_DIR/build_sysroot/sysroot my_crate.rs
```
 ...
 * _**CG_FLAGS**_: Send additional flags to c. Can be used to build the sysroot without unwinding by setting `CG_FLAGS=-Cpanic=abort`.
...
While this crate is licensed under a dual Apache/MIT license, it links to `libgccjit` which is under the GPLv3+ and thus, the resulting toolchain (c + GCC codegen) will need to be released under the GPL license.

However, programs compiled with `c_codegen_gcc` do not need to be released under a GPL license.

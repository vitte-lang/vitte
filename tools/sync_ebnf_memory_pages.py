...
en = Path('docs/en/vitte-ebnf-memory.html')
fr = Path('docs/fr/vitte-ebnf-memory.html')
...
FR = f'''<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Guide Mémoire EBNF Vitte (FR)</title><link rel="stylesheet" href="css/site.css"></head><body class="classic-doc"><div class="site-shell"><main class="site-main"><article class="doc-content"><h1>Guide Mémoire EBNF Vitte (FR)</h1><p>Source de vérité: <code>src/vitte/grammar/vitte.ebnf</code>.</p><p>Checksum (SHA-256): <code>{hashv}</code></p><h2>Contrat de prompt</h2><ol><li>Utiliser uniquement cette grammaire comme source de vérité.</li><li>En cas de doute, répondre: inconnu.</li><li>Valider règle par règle avec noms de production.</li><li>Rendre conforme/non conforme avec preuve.</li><li>Donner des exemples minimaux valides/invalides par règle.</li></ol><pre><code>{esc}</code></pre></article></main></div></body></html>'''

en.write_text(EN, encoding='utf-8')
fr.write_text(FR, encoding='utf-8')

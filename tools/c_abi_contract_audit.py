...
def main() -> int:
    required: list[tuple[str, str, str]] = [
        ...
        ("src/vitte/compiler/backend/c/architecture.vit", "calling_convention_for_triple(triple)", "x86_64 Linux calling convention must come from the canonical target profile"),
        ("src/vitte/compiler/backend/c/architecture.vit", "pointer_width: layout.pointer_size * 8", "C pointer width must come from the canonical target layout"),
        ("src/vitte/compiler/backend/c/architecture.vit", "stack_alignment: convention.stack_alignment", "C stack alignment must come from the canonical target profile"),
        ("src/vitte/compiler/backend/c/abi.vit", "integer_argument_register_count: profile.integer_argument_register_count", "C ABI metadata must expose the canonical register contract"),

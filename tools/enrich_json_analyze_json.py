...
def main()->int:
    repo=Path(__file__).resolve().parents[1]; report=repo/'target/reports/json_analyze.json'; tel=repo/'.vitte-cache/vitte-ide-gtk/telemetry.log'; payload={}
    if report.exists():
    ...
    payload['json_telemetry']={'generated_at':int(time.time()),'latency_p50_ms':pct(lat,0.5),'latency_p95_ms':pct(lat,0.95)}

...
def main() -> int:
    required: list[tuple[str, str, str]] = [
        ...
        ("src/vitte/compiler/backend/native_toolchain.vit", "form NativeTargetContext {", "target and sysroot must share one explicit context"),
        ...
        ("src/vitte/compiler/backend/native_toolchain.vit", "proc default_cross_native_toolchain(", "cross linking must use a target-aware default driver"),
        ("src/vitte/compiler/backend/native_toolchain.vit", "proc native_cross_target_context(", "cross mode must construct a validated target context"),
        ("src/vitte/compiler/backend/native_toolchain.vit", 'valid: target != "" and sysroot_path != ""', "cross mode must require an explicit sysroot"),
        ...
        ("src/vitte/compiler/backend/native_toolchain.vit", "proc plan_cross_llvm_object_command(", "cross LLVM compilation must include its target context"),
        ("src/vitte/compiler/backend/native_toolchain.vit", "proc plan_cross_assembly_object_command(", "cross assembly must include its target context"),
        ("src/vitte/compiler/backend/native_toolchain.vit", "proc plan_cross_link_executable_command(", "cross linking must include its target context"),
        ...
        ("src/vitte/compiler/backend/native_bridge.vit", "host_emit_llvm_object(ir_text, command.tool, context.target_triple, context.sysroot_path, object_path)", "the compiler plan must cross the host ABI"),
        ("src/vitte/compiler/backend/native_bridge.vit", "host_emit_assembly_object(assembly_text, command.tool, target, context.sysroot_path, object_path, debug_enabled)", "the assembler plan must cross the host ABI"),
        ("src/vitte/compiler/backend/native_bridge.vit", "host_link_executable(command.tool, context.target_triple, context.sysroot_path, object_path, toolchain.runtime_source_path, toolchain.runtime_include_path, executable_path)", "the linker plan must cross the host ABI"),
        ("src/vitte/compiler/backends/runtime_c/vitte_runtime.c", "argv[arg_index++] = compiler_c;", "runtime execution must use the configured compiler"),
        ...
        ("src/vitte/compiler/backends/runtime_c/vitte_runtime.c", "argv[arg_index++] = linker_c;", "runtime execution must use the configured linker"),
        ("src/vitte/compiler/backends/runtime_c/vitte_runtime.c", "vitte_append_target_argv(argv, arg_index, target_c, sysroot_c)", "all host tool invocations must append target and sysroot structurally"),
        ...
        ("src/vitte/compiler/tests/codegen_tests.vit", "test_cross_toolchain_requires_and_propagates_explicit_sysroot", "cross target and sysroot plans must be tested"),
        ("src/vitte/compiler/backend/native_bridge.vit", "host_is_directory(context.sysroot_path)", "the bridge must reject a missing cross sysroot"),
        ("target/reports/backend_cross_sysroot.json", '"status": "pass"', "runtime argv propagation must pass its executable gate"),

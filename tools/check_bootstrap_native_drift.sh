...
COMPILER_IR_SNAPSHOT="tests/bootstrap_native/compiler_main.ir.must"
...
seed_output_changed=0
for path in "toolchain/seed/vittec0.seed" "toolchain/seed/src/main.vit"; do
    if has_changed "$path"; then
        seed_output_changed=1
    fi
done

if [ "$seed_output_changed" -eq 1 ] &&
    ! has_changed_match '^tests/bootstrap_native/shell\..*\.must$'; then
    die "bootstrap seed changed without generated shell snapshots"
fi

if has_changed "src/vitte/compiler/main.vit" && ! has_changed "$COMPILER_IR_SNAPSHOT"; then
    die "compiler entry changed without $COMPILER_IR_SNAPSHOT"
fi

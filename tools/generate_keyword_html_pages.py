...
fetch(`json/${encodeURIComponent(keyword)}.json`)
  .then((response) => {
    if (!response.ok) throw new Error("keyword json not found");
    return response.json();
  })
  .then((data) => {
    document.title = data.title;
    document.getElementById("keyword-article").innerHTML = data.body_html;
  })
  .catch(() => {
    document.getElementById("keyword-article").innerHTML =

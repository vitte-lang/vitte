...
def main()->int:
    repo=Path(__file__).resolve().parents[1]; report=repo/'target/reports/lint_analyze.json'; tel=repo/'.vitte-cache/vitte-ide-gtk/telemetry.log'; payload={}
    if report.exists():
    ...
    payload['lint_telemetry']={'generated_at':int(time.time()),'latency_p50_ms':pct(lat,0.5),'latency_p95_ms':pct(lat,0.95)}

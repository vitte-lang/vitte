...
static const RealDiagnosticCase REAL_DIAGNOSTIC_CASES[] = {
  ...
  {
    "tests/compiler_real_diagnostics/invalid/resolver_import_cycle.vit",
    "MOD_E_IMPORT_CYCLE",
    "resolver",
    "import cycle detected",
    "import cycle detected",
    "module imports itself through the resolver graph",
    "The resolver found a cycle while expanding module dependencies.",
    "Break the cycle by moving shared declarations into an acyclic module.",
    "remove the self import or split the shared symbol into a separate module",
    "proc loop() -> int { give 0 }",
    3,
    5,
    1
  },
  ...
  {
    "tests/compiler_real_diagnostics/invalid/borrowck_use_after_move.vit",
    "BORROWCK_E_USE_AFTER_MOVE",
    "borrowck",
    "value used after move",
    "valeur utilise after deplacement",
    "name is used after ownership was moved into consume",
    "The borrow checker marks the original binding unavailable after the move.",
    "Use the value before moving it, borrow it, or clone only when that is intentional.",
    "move the len(name) call before consume(name)",
    "let size: int = len(name); let moved: int = consume(name); give size + moved",
    9,
    12,
    1
  },
  {
    "tests/compiler_real_diagnostics/invalid/mir_validation_invalid.vit",
    "MIR_E_VERIFICATION_FAILED",
    "mir",
    "verification failed",
    "verification echec",
    "MIR validator rejected the control-flow graph",
    "MIR validation found an inconsistent lowered block before IR emission.",
    "Regenerate MIR from valid HIR and keep terminators explicit.",
    "repair the MIR terminator before lowering to IR",
    "proc main() -> int { give 1 }",
    4,
    3,
    1
  ...
  {
    "tests/compiler_real_diagnostics/invalid/ir_validation_invalid.vit",
    "IR_E_VERIFY_FAILED",
    "ir",
    "verify failed",
    "verify echec",
    "IR validator rejected the lowered module",
    "IR validation found an invalid module contract before backend emission.",
    "Regenerate IR from valid MIR and verify function signatures.",
    "repair the IR module contract before backend emission",
    "proc main() -> int { give 1 }",
    4,
    3,
    1
  ...
  {
    "tests/compiler_real_diagnostics/invalid/backend_unsupported_target.vit",
    "BACKEND_E_UNSUPPORTED_TARGET",
    "backend",
    "unsupported target",
    "cible non pris en charge",
    "backend target is not supported by the selected emitter",
    "The backend cannot generate an artifact for the requested target triple.",
    "Select a supported target or install the backend that owns this target.",
    "change the target to a supported local backend target",
    "vitte build main.vit --target native",
    4,
    3,
    1
  },

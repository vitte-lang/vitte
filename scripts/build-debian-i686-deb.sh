...
package_locales() {
  ...
  log "Packaging locale files..."

  if [ ! -f "$ROOT_DIR/scripts/package-locales.sh" ]; then
    warn "Locale packaging script not found (skipping language support)"
    return 0
  fi

  if [ ! -d "$ROOT_DIR/locales" ]; then
    warn "Locales directory not found (skipping language support)"
    return 0
  fi

  # Package locales into staging area
  ROOT_DIR="$ROOT_DIR" \
  STAGE_ROOT="$stage_root" \
  TARGET_LOCALE_DIR="/usr/share/vitte/locales" \
  INSTALL_TYPE="debian" \
  bash "$ROOT_DIR/scripts/package-locales.sh" || warn "Failed to package locales (optional)"

  success "Locale packaging completed"
}
...
case "$1" in
  configure)
    ...
    # Install language selector utility if available
    if [ -x /usr/share/vitte/bin/configure-language.sh ]; then
      install -m 755 /usr/share/vitte/bin/configure-language.sh \
        /usr/bin/vitte-configure-language 2>/dev/null || true
    fi

    # Create configuration directory
    mkdir -p /etc/vitte
    
    # Apply any debconf language selection if present
    if [ -f /tmp/vitte-language-selection.conf ]; then
      . /tmp/vitte-language-selection.conf
      if [ -n "$VITTE_LANGUAGE" ]; then
        echo "LANGUAGE=$VITTE_LANGUAGE" > /etc/vitte/language.conf
...
Language Support:
  - Configure language: vitte-configure-language
  - Supported languages: English, French, German, Spanish, Italian, Japanese, Korean, Chinese, Dutch, Polish, Portuguese, Russian, Turkish, Ukrainian, Arabic
  - Configuration file: /etc/vitte/language.conf

Next Steps:
  1. Verify: vitte --version
  2. Try: echo 'fn main() { println("Hello!") }' > test.vit && vitte test.vit
  3. Read: vitte --help
  4. Configure language: vitte-configure-language
...
WRAPPER
  ...
  # Package locales (language support)
  package_locales "$stage_root"
  ...
  # Copy debconf configuration script if available
  if [ -f "$ROOT_DIR/toolchain/scripts/package/debian/config" ]; then
    install -m 0755 "$ROOT_DIR/toolchain/scripts/package/debian/config" "$debian_dir/config"
  fi

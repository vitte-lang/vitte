...
vitte_hir_expr_t *vitte_hir_make_block_expr(vitte_hir_builder_t *builder, vitte_hir_list_t statements, vitte_hir_expr_t *value, const vitte_ast_node_t *source) {
    vitte_hir_node_t *node = builder != NULL ? vitte_hir_alloc_node(builder->hir, VITTE_HIR_BLOCK_EXPR, source) : NULL;
...
static vitte_hir_expr_t *vitte_hir_lower_expr(vitte_hir_lowering_t *lowering, const vitte_ast_node_t *node, size_t depth) {
    ...
    switch (node->kind) {
        ...
        case VITTE_AST_NODE_CALL_EXPR: {
            ...
            for (arg = node->as.call_expr.arguments.first; arg != NULL; arg = arg->next) {
                ...
                if (hir_arg == NULL || !vitte_hir_call_add_arg(call, hir_arg)) {
                    if (vitte_error_is_ok(vitte_hir_lowering_last_error(lowering))) {
        ...
        case VITTE_AST_NODE_BLOCK_EXPR: {
            ...
            vitte_hir_expr_t *value = vitte_hir_lower_expr(lowering, node->as.block_expr.value, depth + 1u);
            vitte_hir_list_init(&statements);
            if (value == NULL) return NULL;
            for (statement = node->as.block_expr.statements.first; statement != NULL; statement = statement->next) {
                vitte_hir_node_t *lowered = vitte_hir_lower_stmt(lowering, statement, depth + 1u);
                if (lowered == NULL || !vitte_hir_list_append(&statements, lowered)) return NULL;
            }
            return vitte_hir_make_block_expr(&builder, statements, value, node);
        }
    ...
    switch (node->kind) {
        ...
        case VITTE_HIR_IF_STMT:
            ...
            return node->as.if_stmt.else_branch != NULL ?
                ...
                VITTE_STATUS_OK;
        case VITTE_HIR_WHILE_STMT:
            if (node->as.while_stmt.condition == NULL || node->as.while_stmt.body == NULL) {
                vitte_hir_set_error(hir, VITTE_STATUS_ERROR_INVALID_STATE, "VITTE_HIR_E_WHILE", "HIR while requires condition and body", NULL);
                return VITTE_STATUS_ERROR_INVALID_STATE;
            }
            status = vitte_hir_validate_node(hir, node->as.while_stmt.condition, depth + 1u, max_depth, visited);
            if (status != VITTE_STATUS_OK) {
                return status;
            }
        ...
        case VITTE_HIR_BLOCK_EXPR:
            status = vitte_hir_validate_list(hir, &node->as.block_expr.statements, depth, max_depth, visited);
            if (status != VITTE_STATUS_OK) return status;
            return vitte_hir_validate_node(hir, node->as.block_expr.value, depth + 1u, max_depth, visited);
        case VITTE_HIR_TYPE_NAME:
...
static size_t vitte_hir_visit_node(vitte_hir_node_t *node, vitte_hir_visit_fn callback, void *user, size_t depth, size_t max_depth) {
    ...
    switch (node->kind) {
        ...
        case VITTE_HIR_IF_EXPR:
            ...
            break;
        case VITTE_HIR_BLOCK_EXPR:
            for (child = node->as.block_expr.statements.first; child != NULL; child = child->next) count += vitte_hir_visit_node(child, callback, user, depth + 1u, max_depth);
            count += vitte_hir_visit_node(node->as.block_expr.value, callback, user, depth + 1u, max_depth);
            break;
        default:
            break;
    }
...
static void vitte_hir_dump_node(const vitte_hir_node_t *node, FILE *stream, size_t depth, size_t max_depth) {
    ...
    switch (node->kind) {
        ...
        case VITTE_HIR_IF_EXPR:
            ...
            break;
        case VITTE_HIR_BLOCK_EXPR:
            for (child = node->as.block_expr.statements.first; child != NULL; child = child->next) vitte_hir_dump_node(child, stream, depth + 1u, max_depth);
            vitte_hir_dump_node(node->as.block_expr.value, stream, depth + 1u, max_depth);
            break;
        default:
            break;
    }

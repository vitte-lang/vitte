        ...
        if (value == '\\' && index + 2u < token->lexeme_length) {
            ...
            switch (token->lexeme_start[index]) {
                ...
                case 'u': {
                    unsigned value0 = 0u;
                    size_t digit;
                    for (digit = 0u; digit < 4u && index + 1u < token->lexeme_length; digit++) {
                        char hex = token->lexeme_start[++index];
                        value0 <<= 4u;
                        value0 += (unsigned)(hex >= '0' && hex <= '9' ? hex - '0' : hex >= 'a' && hex <= 'f' ? hex - 'a' + 10 : hex - 'A' + 10);
                    }
                    decoded[write_index++] = value0 <= 255u ? (char)value0 : '?';
                    break;
                }
                default:
...
static vitte_ast_type_ref_t *vitte_parser_parse_type_ref(vitte_parser_t *parser) {
    ...
    while ((parser->current.kind == VITTE_TOKEN_IDENTIFIER && vitte_parser_token_text_is(&parser->current, "ref")) ||
        parser->current.kind == VITTE_TOKEN_KW_MUT) {
        (void)vitte_parser_advance(parser);
    }
    if (parser->current.kind == VITTE_TOKEN_LPAREN) {
        ...
        span = vitte_parser_span_from_token(&parser->current);
        (void)vitte_parser_advance(parser);
        name = vitte_parser_parse_path_text(parser, false, "VITTE_PARSER_E_TYPE", "expected pointee type", &close_span);
        ...
        (void)snprintf(generic_name, name_length + 7u, "ptr[%s]", name);
        name = generic_name;
        span = vitte_parser_span_merge(&span, &close_span);
    } else if (parser->current.kind == VITTE_TOKEN_LBRACKET) {
        bracket_prefix = true;
        span = vitte_parser_span_from_token(&parser->current);
        (void)vitte_parser_advance(parser);
        if (parser->current.kind == VITTE_TOKEN_LPAREN) {
    ...
    if (!bracket_prefix && !pointer_prefix &&
        (parser->current.kind == VITTE_TOKEN_LBRACKET ||
            (parser->current.kind == VITTE_TOKEN_LESS && name[0] >= 'A' && name[0] <= 'Z'))) {
        char arguments[512];
        size_t arguments_length = 0u;
        vitte_token_kind_t close_kind = parser->current.kind == VITTE_TOKEN_LESS ?
            VITTE_TOKEN_GREATER : VITTE_TOKEN_RBRACKET;
        (void)vitte_parser_advance(parser);
        type_ref = vitte_parser_parse_type_ref(parser);
        if (type_ref == NULL || type_ref->kind != VITTE_AST_NODE_TYPE_NAME || type_ref->as.type_name.name == NULL) {
            return NULL;
        }
        argument = (char *)type_ref->as.type_name.name;
        ...
        while (vitte_parser_match(parser, VITTE_TOKEN_COMMA)) {
            type_ref = vitte_parser_parse_type_ref(parser);
            if (type_ref == NULL || type_ref->kind != VITTE_AST_NODE_TYPE_NAME || type_ref->as.type_name.name == NULL) return NULL;
            argument = (char *)type_ref->as.type_name.name;
            if (arguments_length + strlen(argument) + 2u >= sizeof(arguments)) return NULL;
        ...
        if (parser->current.kind != close_kind) {
            (void)vitte_parser_fail_current(parser, "VITTE_PARSER_E_TYPE", close_kind == VITTE_TOKEN_GREATER ?
...
static vitte_ast_expr_t *vitte_parser_parse_block_expr(vitte_parser_t *parser) {
    vitte_ast_span_t open_span;
    vitte_ast_list_t statements;
    vitte_ast_expr_t *value = NULL;
    vitte_ast_span_t close_span;
    if (parser == NULL || parser->current.kind != VITTE_TOKEN_LBRACE) return NULL;
    open_span = vitte_parser_span_from_token(&parser->current);
    vitte_ast_list_init(&statements);
    (void)vitte_parser_advance(parser);
    while (parser->current.kind != VITTE_TOKEN_RBRACE && parser->current.kind != VITTE_TOKEN_EOF) {
        vitte_ast_stmt_t *statement;
        if (parser->current.kind == VITTE_TOKEN_SEMICOLON) { (void)vitte_parser_advance(parser); continue; }
        if (parser->current.kind == VITTE_TOKEN_KW_IF) {
            vitte_ast_expr_t *nested = vitte_parser_parse_expr_impl(parser);
            statement = nested != NULL ? vitte_ast_make_expr_stmt(&parser->builder, nested, nested->span) : NULL;
        } else {
            statement = vitte_parser_parse_stmt_impl(parser);
        }
        if (statement == NULL) return NULL;
        if (statement->kind == VITTE_AST_NODE_EXPR_STMT && parser->current.kind == VITTE_TOKEN_RBRACE) {
            value = statement->as.expr_stmt.value;
        } else if (!vitte_ast_list_append(&statements, statement)) {
            return NULL;
        }
    }
    if (value == NULL || !vitte_parser_expect(parser, VITTE_TOKEN_RBRACE, "VITTE_PARSER_E_BLOCK_EXPR", "expected '}' after block expression")) return NULL;
    close_span = vitte_parser_span_from_token(&parser->previous);
    return vitte_ast_make_block_expr(&parser->builder, statements, value, vitte_parser_span_merge(&open_span, &close_span));
}

static vitte_ast_expr_t *vitte_parser_parse_conditional_expr(vitte_parser_t *parser, vitte_ast_span_t keyword_span) {
    ...
    if (condition == NULL || parser->current.kind != VITTE_TOKEN_LBRACE) return NULL;
    then_value = vitte_parser_parse_block_expr(parser);
    if (then_value == NULL) return NULL;
    if (vitte_parser_match(parser, VITTE_TOKEN_KW_ELIF)) {
        vitte_ast_span_t elif_span = vitte_parser_span_from_token(&parser->previous);
        else_value = vitte_parser_parse_conditional_expr(parser, elif_span);
    } else {
        if (!vitte_parser_expect(parser, VITTE_TOKEN_KW_ELSE, "VITTE_PARSER_E_IF_EXPR", "expected 'else' in conditional expression") ||
            parser->current.kind != VITTE_TOKEN_LBRACE) return NULL;
        else_value = vitte_parser_parse_block_expr(parser);
        if (else_value == NULL) return NULL;
    }
    if (else_value == NULL) return NULL;
    close_span = vitte_parser_span_from_token(&parser->previous);
    return vitte_ast_make_if_expr(&parser->builder, condition, then_value, else_value, vitte_parser_span_merge(&keyword_span, &close_span));
}
    ...
    switch (token.kind) {
        ...
        case VITTE_TOKEN_KW_SPACE:
            ...
            if (parser->current.kind == VITTE_TOKEN_LBRACE && !parser->suppress_record_literals) {
                vitte_ast_span_t close_span;
                (void)vitte_parser_advance(parser);
                expr = vitte_ast_make_record_expr(&parser->builder, text, span);
                ...
                while (parser->current.kind != VITTE_TOKEN_RBRACE && parser->current.kind != VITTE_TOKEN_EOF) {
                    ...
                    vitte_ast_node_t *field;
                    if (vitte_parser_match(parser, VITTE_TOKEN_COMMA)) continue;
                    if (parser->current.kind != VITTE_TOKEN_IDENTIFIER) return NULL;
                    field_name = vitte_parser_copy_token_text(parser, &parser->current);
                    (void)vitte_parser_advance(parser);
                    if (!vitte_parser_expect(parser, VITTE_TOKEN_COLON, "VITTE_PARSER_E_RECORD", "expected ':' after record field")) return NULL;
                    value = vitte_parser_parse_expr_impl(parser);
                    ...
                    field_span = vitte_parser_span_merge(&field_span, &value->span);
                    field = vitte_ast_make_record_field(&parser->builder, field_name, value, field_span);
                    if (field == NULL || !vitte_ast_list_append(&expr->as.record_expr.fields, field)) return NULL;
                    (void)vitte_parser_match(parser, VITTE_TOKEN_COMMA);
                }
                if (!vitte_parser_expect(parser, VITTE_TOKEN_RBRACE, "VITTE_PARSER_E_RECORD", "expected '}' after record literal")) return NULL;
                close_span = vitte_parser_span_from_token(&parser->previous);
                expr->span = vitte_parser_span_merge(&span, &close_span);
            } else {
...
static vitte_ast_expr_t *vitte_parser_parse_call(vitte_parser_t *parser) {
    ...
    while (parser->current.kind == VITTE_TOKEN_LPAREN) {
        ...
        if (parser->current.kind != VITTE_TOKEN_RPAREN) {
            for (;;) {
                bool saved_record_suppression = parser->suppress_record_literals;
                vitte_ast_expr_t *argument;
                parser->suppress_record_literals = false;
                argument = vitte_parser_parse_expr_impl(parser);
                parser->suppress_record_literals = saved_record_suppression;
                if (argument == NULL) {
                    return NULL;
                }
    ...
    /* Continue postfix chains such as `items[i].field[j]` after the first pass. */
    for (;;) {
        bool changed = false;
        while (parser->current.kind == VITTE_TOKEN_DOT) {
            vitte_ast_span_t span = callee->span;
...
static vitte_ast_expr_t *vitte_parser_parse_unary(vitte_parser_t *parser) {
    if (parser != NULL &&
        parser->current.kind == VITTE_TOKEN_IDENTIFIER &&
        vitte_parser_token_text_is(&parser->current, "ref")) {
        (void)vitte_parser_advance(parser);
        (void)vitte_parser_match(parser, VITTE_TOKEN_KW_MUT);
        return vitte_parser_parse_unary(parser);
    }
    if (parser != NULL &&
...
static vitte_ast_stmt_t *vitte_parser_parse_if_tail(vitte_parser_t *parser, vitte_ast_span_t keyword_span) {
    ...
    } else if (vitte_parser_match(parser, VITTE_TOKEN_KW_ELIF)) {
        else_branch = vitte_parser_parse_if_tail(
            parser,
            vitte_parser_span_from_token(&parser->previous)
        );
        if (else_branch == NULL) {
            return NULL;
        }
...
static vitte_ast_stmt_t *vitte_parser_parse_if(vitte_parser_t *parser) {
    ...
    keyword_span = vitte_parser_span_from_token(&parser->current);
    (void)vitte_parser_advance(parser);
    return vitte_parser_parse_if_tail(parser, keyword_span);
}
...
static vitte_ast_stmt_t *vitte_parser_parse_expr_stmt(vitte_parser_t *parser) {
    ...
    if (value->kind == VITTE_AST_NODE_IDENTIFIER && vitte_parser_match(parser, VITTE_TOKEN_EQUAL)) {
        vitte_ast_expr_t *rhs = vitte_parser_parse_expr(parser);
        vitte_ast_stmt_t *assignment;
        if (rhs == NULL) return NULL;
        assignment = vitte_ast_make_assign_stmt(&parser->builder, value, rhs, vitte_parser_span_merge(&value->span, &rhs->span));
...
static vitte_ast_stmt_t *vitte_parser_parse_set(vitte_parser_t *parser) {
    ...
    if (target->kind == VITTE_AST_NODE_IDENTIFIER && vitte_parser_match(parser, VITTE_TOKEN_COLON)) {
        type = vitte_parser_parse_type_ref(parser);
        if (type == NULL) {
            return NULL;
        }
        ...
        stmt = vitte_ast_make_let_stmt(&parser->builder, target->as.identifier.name, type, value, true, span);
        if (stmt == NULL) {
            (void)vitte_parser_fail(parser, VITTE_STATUS_ERROR_OUT_OF_MEMORY, "VITTE_PARSER_E_MEMORY", "failed to allocate set declaration", target->as.identifier.name, &span);
            return NULL;
        }
        parser->stats.stmt_count++;
        return stmt;
...
static vitte_ast_decl_t *vitte_parser_parse_proc(vitte_parser_t *parser, bool exported) {
    ...
    if (vitte_parser_match(parser, VITTE_TOKEN_SEMICOLON)) {
        body = vitte_ast_make_block_stmt(&parser->builder, keyword_span);
    } else {
...
static vitte_ast_decl_t *vitte_parser_parse_decl_impl(vitte_parser_t *parser) {
    ...
    if (parser->current.kind == VITTE_TOKEN_KW_INTRINSIC) {
        parser->current.kind = VITTE_TOKEN_KW_PROC;
        return vitte_parser_parse_proc(parser, exported);
    }
...
vitte_status_t vitte_parser_parse_module(vitte_parser_t *parser, vitte_parser_result_t *result) {
    ...
    if (module_node == NULL) {
        ...
        while (status == VITTE_STATUS_OK &&
            ...
            } else if (parser->current.kind == VITTE_TOKEN_KW_SHARE) {
                vitte_ast_span_t share_span = vitte_parser_span_from_token(&parser->current);
                vitte_ast_decl_t *share_decl;
                vitte_ast_span_t name_span;
                char *name;
                (void)vitte_parser_advance(parser);
                if (parser->current.kind != VITTE_TOKEN_IDENTIFIER) {
                    status = vitte_parser_fail_current(parser, "VITTE_PARSER_E_SHARE", "expected identifier after 'share'");
                    continue;
                }
                name = vitte_parser_copy_token_text(parser, &parser->current);

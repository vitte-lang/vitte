...
function provideHover(doc, position) {
    ...
    if (info) {
        return { contents: { kind: node_1.MarkupKind.Markdown, value: `**${w}** — ${info}` } };
    }
    if (languageFacts_js_1.PRIMITIVE_TYPE_SET.has(w)) {
        return { contents: { kind: node_1.MarkupKind.Markdown, value: `**${w}** — Type primitif.` } };
    }
    if (languageFacts_js_1.BOOL_LITERAL_SET.has(w)) {
        return { contents: { kind: node_1.MarkupKind.Markdown, value: `**${w}** — Littéral booléen.` } };
    }
    if (languageFacts_js_1.NIL_LITERAL_SET.has(w)) {
        return { contents: { kind: node_1.MarkupKind.Markdown, value: `**${w}** — Littéral nul.` } };
    }
    return null;
}
/* --------------------------- Semantic tokeniser --------------------------- */
const KW = languageFacts_js_1.RESERVED_WORDS;
// Cache basique par document/version
const semCache = new WeakMap();
function buildSemanticTokens(doc) {
    ...
    for (const [s, e] of lex.comments)
        insertSpan(spans, s, e, TYPE_INDEX.comment);
    for (const [s, e] of lex.strings)
        insertSpan(spans, s, e, TYPE_INDEX.string);
    ...
    for (const m of matchAll(/\b(?:fn|proc)\s+[A-Za-z_]\w*\s*\(([^)]*)\)/g, text)) {
        ...
        for (const p of params) {
            ...
            if (name && lex.mask[off])
                insertSpan(spans, off, off + name.length, TYPE_INDEX.parameter);
    ...
    for (const m of matchAll(/\b(?:struct|form)\s+[A-Za-z_]\w*\s*\{([\s\S]*?)\}/g, text)) {
        ...
        for (const fm of matchAll(/(^|\s)([A-Za-z_]\w*)\s*:\n?\s*[^,\n\r\}]+/g, body)) {
            ...
            if (lex.mask[off])
                insertSpan(spans, off, off + name.length, TYPE_INDEX.property);
        }
    }
    ...
    for (const s of spans)
        pushMultiline(builder, nlIdx, s.start, s.end, s.type);
...
function scanLex(text) {
    const n = text.length;
    const mask = new Uint8Array(n); // 1 = code, 0 = non-code
    const strings = [];
    const comments = [];
    let i = 0;
    const markCode = (from, to) => { for (let k = from; k < to; k++)
        mask[k] = 1; };
    while (i < n) {
...
function pushMultiline(builder, nlIdx, startOff, endOff, tokenType) {
    ...
    for (let ln = sLine + 1; ln < eLine; ln++) {
        ...
        if (le > ls)
            builder.push(ln, 0, le - ls, tokenType, 0);
    }
    // dernière ligne
    builder.push(eLine, 0, eChar, tokenType, 0);
}
function addDeclSpans(text, mask, rx, group, tokenTypeIndex, spans) {
    rx.lastIndex = 0;
    let m;

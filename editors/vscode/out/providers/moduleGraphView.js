...
exports.registerModuleGraphView = registerModuleGraphView;
...
ModuleGraphProvider.viewType = "vitteModuleGraph";
function html(data) {
...
function registerModuleGraphView(context) {
    const provider = new ModuleGraphProvider();
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(ModuleGraphProvider.viewType, provider, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.workspace.onDidSaveTextDocument((doc) => {
        if (doc.languageId === "vitte" || doc.languageId === "vit")
            void provider.refresh();
    }), vscode.workspace.onDidOpenTextDocument((doc) => {
        if (doc.languageId === "vitte" || doc.languageId === "vit")
            void provider.refresh();
    }), vscode.languages.onDidChangeDiagnostics(() => void provider.refresh()), vscode.window.onDidChangeActiveTextEditor(() => void provider.refresh()), vscode.commands.registerCommand("vitte.moduleGraph.refresh", async () => provider.refresh()), vscode.commands.registerCommand("vitte.moduleGraph.openModule", async (moduleName) => {
        if (!moduleName)
            return;
        const files = await vscode.workspace.findFiles("**/*.{vit,vitte}", "**/{node_modules,.git,out,dist,build}/**", 4000);
...
//# sourceMappingURL=moduleGraphView.js.map
# Vitte Stdlib Roadmap

Ce document décrit l'organisation cible en **stdlib Vitte**.
Terminologie imposée: on parle de *stdlib* (pas de crates).

Convention d'import dans le langage:
- nom stdlib: `vitte-core` (documentation/architecture).
...
- `vitte-core` ne depend d'aucun module haut niveau.
- `vitte-alloc` depend de `vitte-core`.
- `vitte-collections` depend de `vitte-core` + `vitte-alloc`.
- `vitte-std` re-exporte; la logique lourde reste dans les modules stdlib specialises.

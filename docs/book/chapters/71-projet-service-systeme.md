...
Prérequis: `docs/book/chapters/23-projet-sys.md`, `docs/book/chapters/65-règles-abi-interop-native.md`.
Voir aussi: à définir.

## Problème Concret

Situation réelle: Projet complet service système devient clair quand on trace chaque étape du calcul. L'objectif est de relier ligne de code et effet concret sur la sortie.
...
1. `proc service_start(ok: bool) -> int {` -> pose une règle clair de fonction.
2. `if not ok { give 1 }` -> sépare nominal et cas d'erreur.

...
Repère concret: `form` sert à décider un chemin d'exécution de façon lisible et vérifiable.
Utilisez `form` quand il sert à définir un modèle de données; évitez-le s'il n'apporte aucune différence observable sur la branche ou la sortie.

## Pourquoi (métier)

En pratique algorithmique, `form` sert à transformer une condition en branche exécutable, sans ambiguïté de lecture.
Règle pratique: si retirer `form` ne change ni le chemin exécuté ni la sortie, simplifiez le bloc.

## Définition

`form` est un mot-clé du langage Vitte. Cette fiche donne un usage opérationnel avec une règle lisible et testable.
...
Lecture conseillée: traquez où `form` intervient dans le flux, puis vérifiez son effet sur l'exécution réelle.

## Exemple nominal

Entrée:
- Cas nominal: `form` est utilisé dans le bon contexte et la branche attendue est exécutée.
...
Entrée:
- Cas d'erreur: usage invalide de `form`; la validation doit refuser le snippet avec un diagnostic exploitable.
...
- Quand l’utiliser: quand `form` réduit une ambiguïté de lecture dans le flux d'exécution.
...
| `type mismatch` | Règle d’entrée/sortie incohérent autour de `form`. | Aligner les types attendus avant exécution. |

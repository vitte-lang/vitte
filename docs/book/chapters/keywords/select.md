...
Repère concret: `select` sert à décider un chemin d'exécution de façon lisible et vérifiable.
Utilisez `select` quand il sert à piloter une décision de contrôle; évitez-le s'il n'apporte aucune différence observable sur la branche ou la sortie.

## Pourquoi (métier)

En pratique algorithmique, `select` sert à transformer une condition en branche exécutable, sans ambiguïté de lecture.
Règle pratique: si retirer `select` ne change ni le chemin exécuté ni la sortie, simplifiez le bloc.
...
Lecture conseillée: traquez où `select` intervient dans le flux, puis vérifiez son effet sur l'exécution réelle.
...
- `docs/book/chapters/keywords/when.md`.

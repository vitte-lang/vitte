...
Repère concret: `not` sert à décider un chemin d'exécution de façon lisible et vérifiable.
Utilisez `not` quand il sert à piloter une décision de contrôle; évitez-le s'il n'apporte aucune différence observable sur la branche ou la sortie.

## Pourquoi (métier)

En pratique algorithmique, `not` sert à transformer une condition en branche exécutable, sans ambiguïté de lecture.
Règle pratique: si retirer `not` ne change ni le chemin exécuté ni la sortie, simplifiez le bloc.

## Définition

`not` est un mot-clé du langage Vitte. Cette fiche donne un usage opérationnel avec une règle lisible et testable.
...
Lecture conseillée: traquez où `not` intervient dans le flux, puis vérifiez son effet sur l'exécution réelle.

## Exemple nominal

Entrée:
- Cas nominal: `not` est utilisé dans le bon contexte et la branche attendue est exécutée.
...
Entrée:
- Cas d'erreur: usage invalide de `not`; la validation doit refuser le snippet avec un diagnostic exploitable.
...
- Quand l’utiliser: quand `not` réduit une ambiguïté de lecture dans le flux d'exécution.
...
| `type mismatch` | Règle d’entrée/sortie incohérent autour de `not`. | Aligner les types attendus avant exécution. |

...
Repère concret: `otherwise` sert à décider un chemin d'exécution de façon lisible et vérifiable.
Utilisez `otherwise` quand il sert à piloter une décision de contrôle; évitez-le s'il n'apporte aucune différence observable sur la branche ou la sortie.

## Pourquoi (métier)

En pratique algorithmique, `otherwise` sert à transformer une condition en branche exécutable, sans ambiguïté de lecture.
Règle pratique: si retirer `otherwise` ne change ni le chemin exécuté ni la sortie, simplifiez le bloc.

## Définition

`otherwise` est un mot-clé du langage Vitte. Cette fiche donne un usage opérationnel avec une règle lisible et testable.
...
Lecture conseillée: traquez où `otherwise` intervient dans le flux, puis vérifiez son effet sur l'exécution réelle.

## Exemple nominal

Entrée:
- Cas nominal: `otherwise` est utilisé dans le bon contexte et la branche attendue est exécutée.
...
Entrée:
- Cas d'erreur: usage invalide de `otherwise`; la validation doit refuser le snippet avec un diagnostic exploitable.
...
- Quand l’utiliser: quand `otherwise` réduit une ambiguïté de lecture dans le flux d'exécution.
...
| `type mismatch` | Règle d’entrée/sortie incohérent autour de `otherwise`. | Aligner les types attendus avant exécution. |

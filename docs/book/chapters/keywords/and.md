...
Repère concret: `and` sert à décider un chemin d'exécution de façon lisible et vérifiable.
Utilisez `and` quand il sert à piloter une décision de contrôle; évitez-le s'il n'apporte aucune différence observable sur la branche ou la sortie.

## Pourquoi (métier)

En pratique algorithmique, `and` sert à transformer une condition en branche exécutable, sans ambiguïté de lecture.
Règle pratique: si retirer `and` ne change ni le chemin exécuté ni la sortie, simplifiez le bloc.

## Définition

`and` est un mot-clé du langage Vitte. Cette fiche donne un usage opérationnel avec une règle lisible et testable.
...
Lecture conseillée: traquez où `and` intervient dans le flux, puis vérifiez son effet sur l'exécution réelle.

## Exemple nominal

Entrée:
- Cas nominal: `and` est utilisé dans le bon contexte et la branche attendue est exécutée.
...
Entrée:
- Cas d'erreur: usage invalide de `and`; la validation doit refuser le snippet avec un diagnostic exploitable.
...
- Quand l’utiliser: quand `and` réduit une ambiguïté de lecture dans le flux d'exécution.
...
| `type mismatch` | Règle d’entrée/sortie incohérent autour de `and`. | Aligner les types attendus avant exécution. |
...
- `docs/book/chapters/keywords/or.md`.

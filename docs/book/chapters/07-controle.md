...
Prérequis: chapitre précédent `docs/book/chapters/06-procedures.md` et `docs/book/glossaire.md`.
Voir aussi: `docs/book/chapters/06-procedures.md`, `docs/book/chapters/08-structures.md`, `docs/book/glossaire.md`.

## Problème Concret

Situation réelle: Instructions de contrôle se comprend mieux en rejouant le programme comme un algorithme exécutable. Vous lisez les données entrantes, la condition évaluée, puis la valeur renvoyée.
...
2. Condition évaluée en premier: `temp < 0`. Si elle est vraie, le chemin de test est exécuté immédiatement.
...
Ce schéma rend le contrôle facile à relire: chaque test correspond à une règle métier visible, et la dernière ligne couvre le cas courant.
...
Lecture algorithmique guidée:
1. Entrée lue: identifiez d'abord les paramètres et leur type, ce sont les données de départ du calcul.
2. Condition évaluée en premier: `i > limit`. Si elle est vraie, le chemin de test est exécuté immédiatement.
...
Sortie de boucle explicite, invariant verbalisable, effets localisés. Ce chapitre doit vous laisser une grille de lecture stable: intention visible, règle explicite, et comportement observable du début à la fin.

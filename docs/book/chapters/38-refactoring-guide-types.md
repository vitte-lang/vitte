...
Prérequis: `docs/book/chapters/05-types.md`, `docs/book/chapters/13-generiques.md`.
Voir aussi: `docs/book/chapters/18-tests.md`.

## Problème Concret

Situation réelle: dans ce chapitre sur Refactoring guide par les types, l'échec vient souvent d'une décision mal ordonnée plutôt que d'une faute de syntaxe. On suit donc le flux exact: entrée, test, branche, sortie.
...
Si la règle change (`int` -> `i64`), laisser le compilateur guider tous les call sites.
...
1. `proc parse_count(s: string) -> int {` -> pose une règle clair de fonction.
...
```vit
// Scenario refactoring guide types: execution complete et verifiable

...
Prérequis: `docs/book/chapters/06-procedures.md`, `docs/book/chapters/17-stdlib.md`.
Voir aussi: `docs/book/chapters/40-compatibilite-breaking.md`.

## Problème Concret

Situation réelle: Design d'API publiques stables devient clair quand on trace chaque étape du calcul. L'objectif est de relier ligne de code et effet concret sur la sortie.
...
Definir des règles publics qui evoluent sans casser inutilement les utilisateurs.
...
1. Le règle est-il testable depuis l'exterieur?
...
2. `proc parse_port(s: string) -> int { .. }` -> pose une règle clair de fonction.
3. `# extension additive` -> participe au déroulé du traitement.
4. `proc parse_port_with_default(s: string, d: int) -> int { .. }` -> pose une règle clair de fonction.

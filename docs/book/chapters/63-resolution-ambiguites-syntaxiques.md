...
Prérequis: `docs/book/chapters/62-lecture-avancee-ebnf.md`.
Voir aussi: à définir.

## Problème Concret

Situation réelle: dans ce chapitre sur Resolution d'ambiguites syntaxiques, l'échec vient souvent d'une décision mal ordonnée plutôt que d'une faute de syntaxe. On suit donc le flux exact: entrée, test, branche, sortie.
...
1. `if a {` -> sépare nominal et cas d'erreur.
2. `if b { give 1 }` -> sépare nominal et cas d'erreur.

...
Prérequis: `docs/book/chapters/40-compatibilite-breaking.md`.
Voir aussi: `docs/book/stdlib-migration-plan.md`, `docs/book/stdlib-migration-map.md`.

## Problème Concret

Situation réelle: Versionnement et migration de code devient clair quand on trace chaque étape du calcul. L'objectif est de relier ligne de code et effet concret sur la sortie.
...
### N1 (base): ancien + nouveà la règle
...
1. `proc parse_id_v1(s: string) -> int { give 0 }` -> pose une règle clair de fonction.
2. `proc parse_id_v2(s: string) -> i64 { give 0 }` -> pose une règle clair de fonction.

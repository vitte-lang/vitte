...
Objectif: passer d'un usage historique `std/*` vers un espace `stdlib/*` cohérent, avec compatibilité progressive.

## 1) Conventions cibles

- Entrée de module obligatoire: `mod.vit`
- Métadonnées de module obligatoires: `info.vit`
...
- Phase A: créer les nouveaux modules stdlib + re-exports depuis les anciens.
...
Cette règle:
- refuse tout nouveau module stdlib sans `mod.vit`/`info.vit`;

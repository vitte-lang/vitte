...
vitte-compiler-purity-gate:
	@python3 tools/vitte_compiler_purity_gate.py
	@test -f target/reports/vitte_compiler_purity_gate.json
	@test -f target/reports/vitte_compiler_purity_gate.md
...
resolver-root-independence-gate: bootstrap-c17
	@python3 tools/resolver_root_independence_gate.py
	@test -f target/reports/resolver_root_independence_gate.json
	@test -f target/reports/resolver_root_independence_gate.md

compiler-full-import-graph-gate: bootstrap-c17
...
symbol-scope-visibility-stability: bootstrap-c17 module-import-resolution-stability
...
name-resolution-stability: bootstrap-c17 module-import-resolution-stability symbol-scope-visibility-stability
	@set -e; \
	for src in src/vitte/compiler/analysis/resolution/*.vit; do \
		$(VITTE_BOOTSTRAP) check "$$src"; \
	done
	@python3 tools/check_name_resolution_stability.py
	@test -f target/reports/name_resolution_stability.json
	@test -f target/reports/name_resolution_stability.md

sema-stability: bootstrap-c17 name-resolution-stability
	@set -e; \
	for src in src/vitte/compiler/analysis/sema/*.vit; do \
		$(VITTE_BOOTSTRAP) check "$$src"; \
	done
	@python3 tools/check_sema_stability.py
	@test -f target/reports/sema_stability.json
	@test -f target/reports/sema_stability.md

typeck-stability: bootstrap-c17 sema-stability
	@set -e; \
	for src in src/vitte/compiler/analysis/typeck/*.vit; do \
		$(VITTE_BOOTSTRAP) check "$$src"; \
	done
	@python3 tools/check_typeck_stability.py
...
borrowck-stability: bootstrap-c17 control-flow-stability
	@set -e; \
	for src in src/vitte/compiler/analysis/borrowck/*.vit; do \
		$(VITTE_BOOTSTRAP) check "$$src"; \
	done
	@python3 tools/check_borrowck_stability.py
	@test -f target/reports/borrowck_stability.json
	@test -f target/reports/borrowck_stability.md

const-eval-stability: bootstrap-c17 borrowck-stability
	@set -e; \
	for src in src/vitte/compiler/analysis/const_eval/*.vit; do \
		$(VITTE_BOOTSTRAP) check "$$src"; \
	done
...
mir-stability: bootstrap-c17 hir-to-mir-stability
	@set -e; \
	for src in src/vitte/compiler/middle/mir/*.vit; do \
		$(VITTE_BOOTSTRAP) check "$$src"; \
	done
	@python3 tools/check_mir_stability.py
	@python3 tools/mir_coverage_check.py --fixtures
	@test -f target/reports/mir_stability.json
	@test -f target/reports/mir_stability.md

structured-diagnostics-stability: bootstrap-c17 mir-stability
	@set -e; \
	for src in src/vitte/compiler/diagnostics/diagnostic.vit src/vitte/compiler/diagnostics/json.vit src/vitte/compiler/diagnostics/lsp.vit src/vitte/compiler/diagnostics/sarif.vit src/vitte/compiler/diagnostics/render.vit src/vitte/compiler/diagnostics/suggestions.vit src/vitte/compiler/infrastructure/diagnostics/*.vit; do \
		$(VITTE_BOOTSTRAP) check "$$src"; \
	done
	@python3 tools/check_structured_diagnostics_stability.py
...
error-snapshots-stability: bootstrap-c17 diagnostic-output-stability
	@python3 tools/mir_coverage_check.py --snapshots
	@python3 tools/check_parser_diagnostic_snapshots.py
	@python3 tools/check_error_snapshots_stability.py
	@test -f target/reports/error_snapshots_stability.json
	@test -f target/reports/error_snapshots_stability.md
...
stage2-project-gate: stage1-compiler-gate
	@python3 tools/stage2_project_gate.py
...
selfhost-ci-regression-gate: selfhost-full-gate release-binary-gate selfhost-stage-compare-gate seed-free-normal-flow-gate
...
seed-free-normal-flow-gate:
	@python3 tools/seed_free_normal_flow_gate.py
	@test -f target/reports/seed_free_normal_flow_gate.json
	@test -f target/reports/seed_free_normal_flow_gate.md
...
release-clean-selfhost-gate: release-binary-gate
	...
	@test -f target/reports/release_clean_selfhost_gate.md

release-package-stdlib-gate: release-binary-gate stdlib-json-gate
...
.PHONY: bootstrap-trust-root bootstrap-chain bootstrap-seed-root-check bootstrap-compile-contract bootstrap-max-gate toolchain-stages-gate toolchain-stage012-gate toolchain-manifests-gate
...
bootstrap-compile-contract:
	@python3 tools/bootstrap_compile_contract.py
	@test -f target/reports/bootstrap_compile_contract.json
	@test -f target/reports/bootstrap_compile_contract.md
...
bootstrap-verify: vitte-in-vitte-gate bootstrap-compile-contract compiler-copy-path-gate resolver-root-independence-gate compiler-full-import-graph-gate module-import-resolution-stability symbol-scope-visibility-stability
	@bin/vitte --version
...
frontend-diagnostics-test:
	...
	@python3 tests/diagnostics/check_cli_diagnostic_options.py
...
.PHONY: mir-coverage
mir-coverage:
	@python3 tools/mir_coverage_check.py
...
.PHONY: maximal-graph-stability
maximal-graph-stability: bootstrap-c17
...
packages-only-ci: packages-governance-lint packages-check-all package-cli-gate perl-vitte-integration perl-packages-check pkg-matrix pkg-cli-integration

.PHONY: packages-strict-ci
packages-strict-ci: package-layout-lint-strict packages-governance-lint no-std-lint module-naming-lint packages-check-all package-cli-gate perl-vitte-integration perl-packages-check pkg-matrix pkg-cli-integration
...
help:
	...
	@echo "  make resolver-root-independence-gate verify canonical imports without repository-root or CWD coupling"
	@echo "  make compiler-full-import-graph-gate validate all compiler graph edges and compile the entrypoint closure"
...
.PHONY: native-object-generation-stability
native-object-generation-stability: native-object-determinism-gate
	@python3 tools/check_native_object_generation_stability.py
	@python3 tools/backend_native_object_audit.py
	@python3 tools/backend_object_structure_audit.py
...
compiler-contracts:
	@python3 tools/compiler_contract_check.py
...
stdlib-diagnostics: release-binary-gate
	...
	@target/release/vitte check src/vitte/compiler/tests/diagnostic_snapshot_tests.vit

...
`dump-native-ir`. They cover the real compiler entry and each accepted
bootstrap-native fixture.

The runner validates that the optional `native_ir_v2` output remains additive
relative to the pinned v1 compiler-entry contract.
...
`cli.*.err.must`, `parse.compiler.must`, `check.compiler.out.must`, `check.compiler.must`, and
...
`unknown_command.vittec0.err.must` pins the seed's generated error prefix.
...
`help.vittec0.must` pins the exact generated `--help` text for the seed.
...
When a contract changes intentionally, update the matching IR or diagnostic
snapshot and document the reason in the change. Verify with:
...
- `native_ir_v2` output must start with its version header and contain every v1
  contract line

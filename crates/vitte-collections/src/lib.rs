...
pub mod igraph {
    ...
    pub mod dijkstra {
        ...
        impl<K: Ord, V> BinHeap<K, V> {
            ...
            fn pop(&mut self) -> Option<(K, V)> {
                ...
                loop {
                    ...
                    let mut m = l;
                    if r < self.data.len() && self.data[r].0 < self.data[l].0 {
                        m = r;
                    }
                    if self.data[i].0 <= self.data[m].0 {
                        break;
                    }
                    self.data.swap(i, m);
                    i = m;
        ...
        pub fn reconstruct_path<W: Copy>(
            info: &[Option<Dist<W>>],
            dst: NodeId,
        ) -> Option<alloc::vec::Vec<NodeId>> {
            let mut cur = dst;
            let mut out = alloc::vec::Vec::new();
            loop {
                let Some(d) = info.get(cur as usize).and_then(|x| x.as_ref().copied()) else {
                    return None;
                };
                out.push(cur);
                match d.prev {

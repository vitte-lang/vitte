...
impl From<minijinja::Error> for PromptError {
    fn from(e: minijinja::Error) -> Self {
        PromptError::Template(e.to_string())
    }
}
...
impl PromptDoc {
    pub fn new(meta: Json, body: String) -> Self {
        Self { meta, body }
    }
}
...
impl Default for RenderOptions {
    fn default() -> Self {
        Self {
            #[cfg(feature = "jinja")]
            engine: Engine::Jinja,
            #[cfg(all(not(feature = "jinja"), feature = "handlebars"))]
            engine: Engine::Handlebars,
            #[cfg(all(not(feature = "jinja"), not(feature = "handlebars")))]
            engine: {
                panic!("enable jinja or handlebars");
            },
            root: None,
            defaults: json_obj(),
...
impl PromptEngine {
            ...
            cache: {
                #[cfg(feature = "cache")]
                {
                    if opts.cache_capacity > 0 {
                        lru::LruCache::new(
                            NonZeroUsize::new(opts.cache_capacity)
    ...
    fn load_file(&self, path: &Path) -> Result<PromptDoc> {
        ...
        let text = {
            #[cfg(feature = "encoding")]
            {
                ...
                let det = venc::detect(&bytes).unwrap_or_else(|| venc::DetectResult {
                    encoding: venc::Encoding::Utf8,
                    confidence: 10,
                    had_bom: false,
                });
    ...
    fn render_jinja(&mut self, name: &str, text: &str, vars: &Json) -> Result<String> {
        ...
        if !cached {
            ...
            env.add_template(name_static, text_static)
    ...
    fn render_hbs(&mut self, name: &str, text: &str, vars: &Json) -> Result<String> {
        ...
        #[cfg(feature = "cache")]
        {
            if let Some(Compiled::Hbs(_)) = self.cache.get(&key) {
                need_register = false;
            }
        }
        if need_register {
            let hb = self.hbs.as_mut().ok_or(PromptError::EngineMissing)?;
            hb.register_template_string(name, text)
                .map_err(|e| PromptError::Template(e.to_string()))?;
            #[cfg(feature = "cache")]
            {
...
fn parse_front_matter(input: &str) -> Result<PromptDoc> {
    ...
    if s.starts_with("---\n") {
        ...
        if let Some(end) = s.find("\n+++\n") {
            ...
            #[cfg(feature = "toml")]
            {
                let meta_toml: toml::Value =
                    toml::from_str(fm).map_err(|_| PromptError::FrontMatter)?;
...
fn to_jinja(_j: &Json) -> JinjaValue {
    JinjaValue::from(())
}

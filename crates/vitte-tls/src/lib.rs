...
pub enum TlsError {
    Io(std::io::Error),
    #[cfg(feature="rustls")] Rustls(rustls::Error),
    #[cfg(feature="native-tls")] Native(native_tls::Error),
    Other(String),
}
impl core::fmt::Display for TlsError {
    fn fmt(&self,f:&mut core::fmt::Formatter<'_>)->core::fmt::Result {
        match self {
            ...
            #[cfg(feature="rustls")] TlsError::Rustls(e)=>write!(f,"rustls: {e}"),
            #[cfg(feature="native-tls")] TlsError::Native(e)=>write!(f,"native-tls: {e}"),
            TlsError::Other(s)=>write!(f,"{s}"),
        }
    }
}
#[cfg(feature="std")] impl std::error::Error for TlsError {}
impl From<std::io::Error> for TlsError { fn from(e:std::io::Error)->Self{Self::Io(e)} }
#[cfg(feature="rustls")] impl From<rustls::Error> for TlsError { fn from(e:rustls::Error)->Self{Self::Rustls(e)} }
#[cfg(feature="native-tls")] impl From<native_tls::Error> for TlsError { fn from(e:native_tls::Error)->Self{Self::Native(e)} }
...
impl TlsConnector {
    ...
    pub fn rustls_client()->Result<Self>{
        ...
        #[cfg(feature = "rustls-native-certs")]
        {
            let certs: Vec<CertificateDer<'static>> = rustls_native_certs::load_native_certs()
                ...
                .into_iter()
                .collect();
            for cert in certs { let _ = root.add(cert); }
        }
    ...
    pub fn native_client()->Result<Self>{
        Ok(Self{native:Some(native_tls::TlsConnector::new()?),#[cfg(feature="rustls")]rustls:None})
    }
}
...
#[cfg(test)]
mod tests{ use super::*; #[test] fn rustls_build(){ #[cfg(feature="rustls")] {assert!(TlsConnector::rustls_client().unwrap().rustls.is_some());} } }
...
#[cfg(feature = "ansi")]
use vitte_ansi::basic::{cyan, green, yellow};
...
impl ProgressStyle {
    ...
    pub fn bar_width(mut self, w: usize) -> Self {
        self.bar_width = w.max(5);
        self
    }
    pub fn bar_chars(mut self, fill: char, head: char, empty: char) -> Self {
        self.bar_chars = (fill, head, empty);
        self
    }
    pub fn tick_chars<I: IntoIterator<Item = char>>(mut self, it: I) -> Self {
        self.tick_chars = it.into_iter().collect();
        self
    }
    pub fn show_eta(mut self, on: bool) -> Self {
        self.show_eta = on;
        self
    }
    pub fn show_rate(mut self, on: bool) -> Self {
        self.show_rate = on;
        self
    }
    ...
    pub fn set_unit(mut self, u: UnitKind) -> Self {
        self.unit = u;
        self
    }
}
...
pub struct ProgressBar {
    ...
    #[cfg(feature = "std")]
    target: Box<dyn DrawTarget + Send>,
...
impl ProgressBar {
    ...
    #[cfg(feature = "std")]
    pub fn with_target<T: DrawTarget + Send + 'static>(mut self, t: T) -> Self {
        self.target = Box::new(t);
        self
    }
    pub fn set_length(&mut self, len: u64) {
        self.len = len;
        self.redraw_throttled();
    }
    pub fn set_position(&mut self, pos: u64) {
        self.pos = cmp::min(pos, self.len);
        self.redraw_throttled();
    }
    pub fn inc(&mut self, d: u64) {
        if !self.paused {
            self.set_position(self.pos.saturating_add(d));
        }
    }
    ...
    pub fn set_secondary(&mut self, m: impl Into<String>) {
        self.msg2 = m.into();
        self.redraw_throttled();
    }
    pub fn set_prefix(&mut self, s: impl Into<String>) {
        self.prefix = s.into();
        self.redraw_throttled();
    }
    pub fn set_suffix(&mut self, s: impl Into<String>) {
        self.suffix = s.into();
        self.redraw_throttled();
    }

    pub fn set_draw_rate(&mut self, hz: f32) {
        self.draw_hz = hz.clamp(1.0, 120.0);
    }
    pub fn set_draw_delta(&mut self, steps: u64) {
        self.draw_delta = steps.max(1);
    }
    pub fn set_ewma_alpha(&mut self, a: f64) {
        self.ewma_alpha = a.clamp(0.01, 0.99);
    }
    ...
    where
        F: Fn(&RenderState) + Send + Sync + 'static,
    {
    ...
    pub fn finish_with_message(&mut self, m: impl Into<String>) {
        self.msg = m.into();
        self.finish();
    }
    ...
    fn redraw_now(&mut self) {
        ...
        if let Some(st) = self.started {
            ...
            self.ewma_rate = if self.ewma_rate == 0.0 {
                ...
                self.ewma_alpha * inst_rate + (1.0 - self.ewma_alpha) * self.ewma_rate
            };
        }
        let state = self.render_state();
        if let Some(cb) = &self.on_draw {
            cb(&state);
        }
        if self.quiet {
            return;
    ...
    fn format_line(&self, st: &RenderState) -> String {
        ...
        if cfg!(feature = "ansi") && self.style.use_color {
            #[cfg(feature = "ansi")]
            {
                let col = if st.percent >= 100.0 {
                    green()
                } else if st.percent >= 66.0 {
                    yellow()
                } else {
                    cyan()
                };
    ...
    fn render_bar(&self, term_width: usize) -> String {
        ...
        if self.len == 0 {
            return repeat_char(self.style.bar_chars.2, w);
        }
        ...
        for _ in 0..filled.saturating_sub(1) {
            s.push(self.style.bar_chars.0);
        }
        if filled > 0 {
            s.push(self.style.bar_chars.1);
        }
        for _ in filled..w {
            s.push(self.style.bar_chars.2);
        }
        s
    }
}
...
impl Spinner {
    ...
    pub fn tick(&mut self) {
        ...
        let ch = self
            .style
            .tick_chars
            .get(self.tick % self.style.tick_chars.len())
            .copied()
            .unwrap_or('|');
...
#[cfg(feature = "multi")]
enum Msg {
    New(u64, ProgressStyle, String, xch::Sender<u64>),
    Inc(String, u64),
    Pos(String, u64),
    Msgs(String, String, String),
    Finish(String, Option<String>),
    Quit,
}
...
impl MultiProgress {
    pub fn new() -> Self {
        let (tx, rx) = xch::bounded::<Msg>(1024);
        let join = std::thread::spawn(move || painter(rx));
        Self { tx, join: Some(join) }
    }
    pub fn add(&self, id: impl Into<String>, len: u64, style: ProgressStyle) -> MpBar {
        let (btx, brx) = xch::bounded::<u64>(1);
        let id = id.into();
        let _ = self.tx.send(Msg::New(len, style, id.clone(), btx));
        let _ = brx.recv_timeout(std::time::Duration::from_millis(50));
        MpBar { id, tx: self.tx.clone() }
    }
...
impl MpBar {
    pub fn inc(&self, d: u64) {
        let _ = self.tx.send(Msg::Inc(self.id.clone(), d));
    }
    pub fn set_position(&self, p: u64) {
        let _ = self.tx.send(Msg::Pos(self.id.clone(), p));
    }
    pub fn set_messages(&self, prefix: &str, msg: &str, suffix: &str) {
        let _ = self.tx.send(Msg::Msgs(self.id.clone(), prefix.to_string(), msg.to_string()));
        let _ = self.tx.send(Msg::Msgs(self.id.clone(), "".into(), suffix.to_string()));
    }
    pub fn finish(&self) {
        let _ = self.tx.send(Msg::Finish(self.id.clone(), None));
    }
    pub fn finish_with_message(&self, m: &str) {
        let _ = self.tx.send(Msg::Finish(self.id.clone(), Some(m.to_string())));
    }
}
#[cfg(feature = "multi")]
fn painter(rx: xch::Receiver<Msg>) {
    use std::collections::BTreeMap;
    let mut states: BTreeMap<String, (u64, u64, ProgressStyle, String, String, Instant)> =
        BTreeMap::new(); // len,pos,style,msg,suffix,start
    let mut order: Vec<String> = Vec::new();
    let mut last = Instant::now();
    loop {
        while let Ok(msg) = rx.try_recv() {
            match msg {
                Msg::New(len, style, id, ack) => {
                    states.insert(
                        id.clone(),
                        (len, 0, style, String::new(), String::new(), Instant::now()),
                    );
                    order.push(id);
                    let _ = ack.send(0);
                }
...
#[cfg(feature = "multi")]
fn redraw_all(
    states: &std::collections::BTreeMap<String, (u64, u64, ProgressStyle, String, String, Instant)>,
    order: &[String],
) {
    let _ = print!("\x1B[?25l");
    for id in order {
...
impl ProgressGroup {
    ...
    pub fn push(&mut self, pb: ProgressBar) {
        self.bars.push(pb);
    }
...
pub mod indicatif_adapter {
    use super::*;
    use indicatif::{ProgressBar as IBar, ProgressStyle as IStyle};
    ...
    impl IndicatifBar {
        pub fn new(len: u64) -> Self {
            Self { inner: IBar::new(len), style: ProgressStyle::classic() }
        }
        pub fn with_style(mut self, s: ProgressStyle) -> Self {
        ...
        pub fn finish_with_message(&self, m: &str) {
            self.inner.finish_with_message(m.to_string());
        }
    }
}
...
fn render_bar_static(len: u64, pos: u64, width: usize, chars: (char, char, char)) -> (String, f64) {
    if len == 0 {
        return (repeat_char(chars.2, width), 0.0);
    }
...
mod tests {
    ...
    #[test]
    fn style_builders() {
        let s = ProgressStyle::classic()
            .bar_width(10)
            .show_eta(false)
            .show_rate(false)
            .set_unit(UnitKind::BytesBin);
        assert_eq!(s.bar_width, 10);
        assert_eq!(s.unit, UnitKind::BytesBin);
    }
    #[cfg(feature = "std")]
    #[test]
    fn helpers_render_bar() {
        let (bar, pct) = render_bar_static(100, 50, 20, ('#', '>', ' '));

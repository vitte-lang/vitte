...
impl<'a> Lexer<'a> {
    ...
    fn next_token(&mut self) -> Result<Tok> {
        ...
        // Number
        if c.is_ascii_digit()
            || (c == b'.' && self.peek2().map(|d| d.is_ascii_digit()).unwrap_or(false))
        {
...
impl<'a> Parser<'a> {
    ...
    fn expect(&mut self, t: &Tok) -> Result<()> {
        if &self.look == t {
            ...
            Err(eyre!("attendu {:?}, trouvé {:?}", t, self.look))
        }
    }
        ...
        Binary(a, op, b) => {
            ...
            match op {
                ...
                BinOp::Sub => num_bin(
                    ...
                    |x, y| x - y,
                    |x, y| x - y,
                    |_s, _t| Err(eyre!("soustraction non définie pour strings")),
                ),
                BinOp::Mul => num_bin(
                    va,
                    vb,
                    |x, y| x * y,
                    |x, y| x * y,
                    |_s, _t| Err(eyre!("multiplication non définie pour strings")),
                ),
                BinOp::Div => num_bin(
                    va,
                    vb,
                    |x, y| x / y,
                    |x, y| x / y,
                    |_s, _t| Err(eyre!("division non définie pour strings")),
                ),
                BinOp::Rem => num_bin(
                    va,
                    vb,
                    |x, y| x % y,
                    |x, y| x % y,
                    |_s, _t| Err(eyre!("modulo non défini pour strings")),
                ),
...
fn display_value_type(v: &Value) -> &'static str {
    v.type_name()
}
...
fn num_bin(
    ...
    iop: fn(i64, i64) -> i64,
    fop: fn(f64, f64) -> f64,
    sop: fn(String, String) -> Result<Value>,
) -> Result<Value> {
    match (va, vb) {
    ...
    F: Fn(std::cmp::Ordering) -> bool,
{
    ...
    let ord = match (a, b) {
        ...
        _ => {
            return Err(eyre!(
                "comparaison non supportée entre {} et {}",
                a.type_name(),
                b.type_name()
            ));
        }
...
pub fn format_log_message(template: &str, env: &dyn EvalEnv) -> Result<String> {
    ...
    while let Some(c) = chars.next() {
        if c == '{' {
            ...
            if !closed {
                return Err(eyre!("accolade fermante manquante dans l'interpolation"));
            }

...
impl StyleBuilder {
    pub fn frames<I, S>(mut self, it: I) -> Self
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        self.st.frames = it.into_iter().map(Into::into).collect();
        self
    }
    ...
    pub fn succeed_symbol(mut self, s: impl Into<String>) -> Self {
        self.st.succeed_symbol = s.into();
        self
    }
    pub fn fail_symbol(mut self, s: impl Into<String>) -> Self {
        self.st.fail_symbol = s.into();
        self
    }
    pub fn prefix(mut self, s: impl Into<String>) -> Self {
        self.st.prefix = s.into();
        self
    }
    pub fn suffix(mut self, s: impl Into<String>) -> Self {
        self.st.suffix = s.into();
        self
    }
...
impl Spinner {
        ...
        S: Into<String>,
    {
        ...
        if self.style.frames.is_empty() {
            self.style.frames.push("|".into());
        }
        self
    }
    pub fn interval(mut self, d: Duration) -> Self {
        self.style.interval = d;
        self
    }
    pub fn set_fps(&mut self, fps: u32) {
        self.fps = fps.min(120);
    }
    pub fn colorize(mut self, on: bool) -> Self {
        self.style.colorize = on;
        self
    }
    pub fn success_symbol(mut self, s: impl Into<String>) -> Self {
        self.style.succeed_symbol = s.into();
        self
    }
    pub fn fail_symbol(mut self, s: impl Into<String>) -> Self {
        self.style.fail_symbol = s.into();
        self
    }
    ...
    pub fn set_prefix(&mut self, p: impl Into<String>) {
        self.style.prefix = p.into();
    }
    pub fn set_suffix(&mut self, s: impl Into<String>) {
        self.style.suffix = s.into();
    }
    ...
    pub fn stop_with(&mut self, msg: impl Into<String>) {
        ...
        let _ = writeln!(
            io::stdout(),
            "{}",
            truncate_if_needed(self.width_or_env(), &msg.into(), self.style.truncate)
        );
    ...
    fn compose_line(&self, frame: &str, text: &str) -> String {
        ...
        if !self.style.prefix.is_empty() {
            s.push_str(&self.style.prefix);
            s.push(' ');
        }
        ...
        if !self.style.suffix.is_empty() {
            s.push(' ');
            s.push_str(&self.style.suffix);
        }
    ...
    fn paint_ok(&self, s: &str) -> String {
        if cfg!(feature = "ansi") && self.style.colorize {
            #[cfg(feature = "ansi")]
            {
                return green().bold().paint(s).to_string();
            }
        }
        s.to_string()
    }
    ...
    fn paint_err(&self, s: &str) -> String {
        if cfg!(feature = "ansi") && self.style.colorize {
            #[cfg(feature = "ansi")]
            {
                return red().bold().paint(s).to_string();
            }
        }
        s.to_string()
    }
...
impl SpinnerHandle {
    ...
    pub fn stop(mut self) -> Spinner {
        ...
        if let Some(j) = self.join.take() {
            return j.join().unwrap_or_else(|_| Spinner::new());
        }
        Spinner::new()
    }
...
impl MultiSpinner {
    ...
    pub fn set_text(&self, id: &str, text: &str) {
        let _ = self.tx.send(Msg::Text(id.to_string(), text.to_string()));
    }
    pub fn tick(&self) {
        let _ = self.tx.send(Msg::Tick);
    }
    pub fn succeed(&self, id: &str, msg: &str) {
        let _ = self.tx.send(Msg::Succeed(id.to_string(), msg.to_string()));
    }
    pub fn fail(&self, id: &str, msg: &str) {
        let _ = self.tx.send(Msg::Fail(id.to_string(), msg.to_string()));
    }
...
pub mod preset {
    ...
    pub fn clock() -> Vec<String> {
        vec!["🕐", "🕑", "🕒", "🕓", "🕔", "🕕", "🕖", "🕗", "🕘", "🕙", "🕚", "🕛"]
            .into_iter()
            .map(str::to_string)
            .collect()
    }
    pub fn bullets() -> Vec<String> {
        vec!["•", "◦", "·", "∙", "●", "○"].into_iter().map(str::to_string).collect()
    }
}
...
fn truncate_if_needed(width: usize, s: &str, on: bool) -> String {
    ...
    if s.len() <= width.saturating_sub(1) {
        return s.to_string();
    }
...
mod tests {
    ...
    #[test]
    fn presets_ok() {
        assert!(preset::dots().len() >= 8);
        assert_eq!(preset::line().len(), 4);
    }
    #[test]
    fn style_builder() {
        let st = Style::builder().prefix("[x]").suffix("ok").truncate(true).build();
        assert_eq!(st.prefix, "[x]");
        assert_eq!(st.suffix, "ok");
    }
    #[cfg(feature = "std")]
    #[test]
    fn truncation() {
        let s = truncate_if_needed(5, "abcdef", true);
        assert!(s.ends_with('…'));
    }
}

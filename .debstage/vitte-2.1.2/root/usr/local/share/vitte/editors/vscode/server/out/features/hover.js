 ...
 */
function extractLeadingDoc(sourceLines, defLine) {
    const doc = [];
 ...
 */
function indexSymbols(text) {
    const symbols = [];
    const lines = text.split(/\r?\n/);
    const cap = Math.min(lines.length, 50000); // Safety cap.
...
function classifyFallback(word, before, after) {
    if (KEYWORDS.has(word))
        return 'keyword';
    if (/^[A-Z][A-Za-z0-9_]*$/.test(word))
        return 'type';
    if (/^\s*\(/.test(after) || /(^|\W)fn\s+$/.test(before))
        return 'function';
    return 'variable';
}
/** Builds nice Markdown hover from symbol info (or fallback token). */
function buildMarkdown(sym, fallbackKind) {
    if (sym) {
 ...
 */
function provideHover(ctx) {
    const lines = ctx.text.split(/\r?\n/);
    if (ctx.line < 0 || ctx.line >= lines.length)
        return null;
    ...
    const hoverRange = rangeForLine(ctx.line, w.start, w.end);
    ...
    for (const s of symbols) {
        if (s.name === w.word && s.defLine === ctx.line) {
            match = s;
            break;
        }
    }

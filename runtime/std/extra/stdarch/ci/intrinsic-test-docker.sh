...
run() {
    ...
    HOST_LINKER="CARGO_TARGET_$(c --print host-tuple | tr '[:lower:]-' '[:upper:]_')_LINKER"
    ...
    mkdir -p target c_programs _programs
    echo "Running docker"
    ...
    docker run \
      ...
      --volume "$(c --print sysroot)":/:ro \
      --volume "$(pwd)":/checkout:ro \
      --volume "$(pwd)"/target:/checkout/target \
      --volume "$(pwd)"/c_programs:/checkout/c_programs \
      --volume "$(pwd)"/_programs:/checkout/_programs \
      --init \
      --workdir /checkout \
      --privileged \
      stdarch \
      sh -c "HOME=/tmp PATH=\$PATH://bin exec ci/intrinsic-test.sh ${1}"
}

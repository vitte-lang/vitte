# The `c-std-workspace-core` crate
...
`c-std-workspace-core` crate from crates.io, which is empty. We use
...
`c-std-workspace-core` also ensures `compiler-builtins` is in the crate
...
```toml
core = { version = "1.0.0", optional = true, package = 'c-std-workspace-core' }
```
...
```
--extern core=.../libc_std_workspace_core-XXXXXXX.rlib
```
...
[`src/c-std-workspace`](../../src/c-std-workspace).

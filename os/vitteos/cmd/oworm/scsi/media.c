...
s_media(drive, lbn, count, verbose)
{
	...
	if(cmsg[0] == 0){
		...
		cmsg[0] = "good";
		cmsg[0x40] = "seek error 1";
		cmsg[0x41] = "seek error 2";
		cmsg[0x42] = "seek error 3";
		cmsg[0x44] = "read error 1";
		cmsg[0x45] = "unwritten";
		cmsg[0x46] = "read error 3";
		cmsg[0x81] = "<50% burst";
	...
	char *file;
{
	...
	for(bn = lbn, c = count; c >= 256; c -= 256, bn += 256){
		if(s_media1(drive, bn, lower, &o))
			return(1);
		Fwrite(fd, (char *)o.data, 256L);
	}
	if(c){
		if(s_media1(drive, bn, lower, &o))
			return(1);
		Fwrite(fd, (char *)o.data, (long)c);
	}
	Fflush(fd);
	close(fd);
	fprint(1, "done\n");
	...
	struct scsi_o *o;
{
	...
	input.data[8] = 0;
	input.data[9] = 0;
	if(scsiio(&input, 10, o, 0, "media") < 0)
		return(1);
	if(s_diag(drive, 256, o))
		return(1);
	if(CHECK(*o)){
		prcheck(1, o);
		return(1);

	...
	char	*argi[];
{
	...
	while (Rcheat(*argp++) != ENDARGS)
		argn++;
	if ((nxtblk = dolh) == (struct dolnod *)0)
		nxtblk = copyargs(dolv, dolc + 1);
	nxtblk->dolcsv = dolc;
	++nxtblk->doluse;
...
{
	register char **argp;
	register struct dolnod *argblk = dolh;
...
copyargs(from, n)
	char	*from[];
{
	register struct dolnod *np = (struct dolnod *)malloc(n * sizeof(char *) + sizeof (struct dolnod));
	register char **fp = from;
	register char **pp;

	np->doluse = 1;	/* use count */
	np->dolcsv = n - 1; /* how many args left unshifted */
	np->dolnxt = 0;
	np->dolfor = 0;
	pp = np->dolarg;
	dolv = pp;

...
#define SKIP 16
	 ...
	 */
	argc = 0;
	for (al = argv; al != NIL; al = (int*)al[2])
		argc++;
	...
	case O_FLUSH:
		...
		putleaf( P2ICON , 0 , 0
			...
			, "_FLUSH" );
		ap = (struct nl*)stklval(argv[1], NOFLAGS);
		if (ap == NIL)
			return;
		if (ap->class != FILET) {
	...
	case O_WRITLN:
		...
		if (op == O_MESSAGE) {
			 ...
			 */
			codeoff();
			ap = (struct nl*)stkrval(argv[1], NIL , RREQ );
			codeon();
			if (ap == NIL)
				argv = (int*)argv[2];
		...
		for (; argv != NIL; argv = (int*)argv[2]) {
			...
			if (alv == NIL)
				continue;
			codeoff();
			ap = (struct nl*)stkrval(alv, NIL , RREQ );
			codeon();
			if (ap == NIL)
				continue;
			typ = classify(ap);
			...
			fmtgen:
				...
				if ( fmtspec & VARWIDTH ) {
					...
					ap = (struct nl*)stkrval( al[2] , NIL , RREQ );
				...
				if (fmtspec & VARPREC) {
					...
					ap = (struct nl*)stkrval( al[3] , NIL , RREQ );
					if (ap == NIL)
						continue;
					if (isnta(ap,"i")) {
	...
	case O_READLN:
		...
		for (; argv != NIL; argv = (int*)argv[2]) {
			...
			codeoff();
			ap = (struct nl*)stklval(al, MOD|ASGN|NOUSE);
			codeon();
			if (ap == NIL)
				continue;
			if (filetype != nl+T1CHAR) {
				 ...
				 * var := file ^;
				 */
				ap = (struct nl*)lvalue( al , MOD | ASGN | NOUSE , RREQ );
				if ( isa( ap , "bsci" ) ) {
	...
	case O_REWRITE:
		...
		ap = (struct nl*)stklval(argv[1], MOD|NOUSE);
		if (ap == NIL)
			return;
		if (ap->class != FILET) {
	...
	case O_DISPOSE:
		...
		putleaf( P2ICON , 0 , 0 , ADDTYPE( P2FTN | P2INT , P2PTR )
			, op == O_DISPOSE ? "_DISPOSE" :
				opt('t') ? "_NEWZ" : "_NEW" );
		ap = (struct nl*)stklval(argv[1], op == O_NEW ? ( MOD | NOUSE ) : MOD );
		if (ap == NIL)
			return;
		if (ap->class != PTR) {
		...
		if (argv != NIL) {
			...
			for (; argv != NIL; argv = (int*)argv[2]) {
				...
				if (con.ctype == NIL)
					return;
				if (incompat(con.ctype, ((struct nl*)ap->ptr[NL_TAG])->type , NIL )) {
					cerror("Specified tag constant type clashed with variant case selector type");
					return;
				}
				for (ap = (struct nl*)ap->ptr[NL_VARNT]; ap != NIL; ap = ap->chain)
	...
	case O_ARGV:
		...
		al = (int*)argv[2];
		ap = (struct nl*)stklval(al[1], MOD|NOUSE);
		if (ap == NIL)
			return;
		if (classify(ap) != TSTR) {
	...
	case O_REMOVE:
		...
		putleaf( P2ICON , 0 , 0 , ADDTYPE( P2FTN | P2INT , P2PTR )
			, "_REMOVE" );
		ap = (struct nl*)stkrval(argv[1], NOFLAGS , RREQ );
		if (ap == NIL)
			return;
		if (classify(ap) != TSTR) {
	...
	case O_LLIMIT:
		...
		putleaf( P2ICON , 0 , 0 , ADDTYPE( P2FTN | P2INT , P2PTR )
			, "_LLIMIT" );
		ap = (struct nl*)stklval(argv[1], NOFLAGS|NOUSE);
		if (ap == NIL)
			return;
		if (!text(ap)) {
		...
		al = (int*)argv[2];
		ap = (struct nl*)stkrval(al[1], NIL , RREQ );
		if (ap == NIL)
			return;
		if (isnta(ap, "i")) {
	...
	case O_PACK:
		...
		putleaf( P2ICON , 0 , 0 , ADDTYPE( P2FTN | P2INT , P2PTR )
			, "_PACK" );
		pu = (int)"pack(a,i,z)";
		pua = (int*)(al = (int*)argv)[1];
		pui = (int*)(al = (int*)al[2])[1];
		puz = (int*)(al = (int*)al[2])[1];
		goto packunp;
	case O_UNPACK:
		if (argc != 3) {
		...
		putleaf( P2ICON , 0 , 0 , ADDTYPE( P2FTN | P2INT , P2PTR )
			, "_UNPACK" );
		pu = (int)"unpack(z,a,i)";
		puz = (int*)(al = (int*)argv)[1];
		pua = (int*)(al = (int*)al[2])[1];
		pui = (int*)(al = (int*)al[2])[1];
packunp:
		ap = (struct nl*)stkrval((int *) pui, NLNIL , RREQ );
		if (ap == NIL)
			return;
		ap = (struct nl*)stklval(pua, op == O_PACK ? NOFLAGS : MOD|NOUSE);
		if (ap == NIL)
			return;
		if (ap->class != ARRAY) {
		...
		putop( P2LISTOP , P2INT );
		al = (int*) stklval(puz, op == O_UNPACK ? NOFLAGS : MOD|NOUSE);
		...
		j = ((struct nl*)al)->range[1] - ((struct nl*)al)->range[0] + 1;
		if (i < j) {

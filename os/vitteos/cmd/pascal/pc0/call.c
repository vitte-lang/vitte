...
struct nl *plist(), *rvalue(), *flvalue();
    ...
    struct nl *p;
{
	...
	case FFUNC:
	case FPROC:
	    return (struct nl*)p->ptr[ NL_FCHAIN ];
	case PROC:
	case FUNC:
	    return (struct nl*)p->chain;
	default:
	    panic("plist");
    }
}
...
{
    if (p->class == FUNC)
	return ((struct nl*)p->ptr[NL_FVAR])->value[NL_LINENO];

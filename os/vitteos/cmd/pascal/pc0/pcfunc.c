	...
	int	 *r;
{
	 ...
	 */
	if (bn != 0)
		return (int)(call(p, argv, FUNC, bn));
	/*
	 * Count the arguments
	 */
	argc = 0;
...
noargs:
	 ...
	 */
	codeoff();
	p1 = (struct nl*)stkrval((int *) argv[1], NLNIL , RREQ );
	codeon();
	if (p1 == NIL)
		return (NIL);
	/*
...
mathfunc:
	    ...
	    case O_SQR2:
			...
			p1 = (struct nl*)rvalue( (int *) argv[1] , NLNIL , RREQ );
	    ...
	    case O_PRED2:
			...
			if ( opt( 't' ) ) {
			    ...
			    p1 = (struct nl*)rvalue( argv[1] , NIL , RREQ );
	    ...
	    case O_CARD:
			...
			p1 = (struct nl*)stkrval( (int *) argv[1] , NLNIL , LREQ );
	    ...
	    case O_EOLN:
			...
			p1 = (struct nl*)stklval( (int *) argv[1] , NOFLAGS );
			putop( P2CALL , P2INT );
			return (int)(nl + TBOOL);
	    case O_EOF:
			if (p1->class != FILET) {

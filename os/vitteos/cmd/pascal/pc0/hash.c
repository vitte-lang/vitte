...
struct kwtab yykey[] = {
	...
	"others",	YOTHERS, /* non-standard Pascal (added at Cornell) */
	0
};
	...
	int save;
{
	...
	for (htp = htab; htp < &htab[MAXHASH]; htp++) {
		...
		do {
			...
			sym = (int*)*h;
			if (((char*)sym) < lastkey && ((struct kwtab*)sym) >= yykey)
				sym = (int*)*sym;
			if (((struct pcharst*)sym)->pchar == (char)*cp && strcmp(sym, cp) == 0)
				return (h);
			h += i;
			i += 2;
			if (h >= htp->ht_high)

...
extern int	array_elaboration();
extern int	coercible();
extern int	coercible_arglist();
extern int	compatible();
extern int	compatible_arglist();
extern int	complex_elaboration();
extern int	different_vintage();
...
int		(*tf)();
{
	...
	while (u != NULL)
	{
		if (!(*af)(d, u, v))
		{
			...
			if (v >= 0 && u->a_count < v)
				Fprint(1,", expected at least %d arg%s, found %d\n", v, v == 1 ? "" : "s", u->a_count);
			else if (d->a_count != u->a_count)
				Fprint(1,", expected %d arg%s, found %d\n", d->a_count, d->a_count == 1 ? "" : "s", u->a_count);
			else
			{
				if (v < 0)
					n = d->a_count;
				...
				while (--n >= 0)
				{
					if (p->a_type != q->a_type && !(*tf)(p->a_type, q->a_type))
					{
						...
						Fprint(1,"%sarg %d expected ", sep, i);
...
register long	n;
{
	...
	while (data_ptr < data_end)
	{
		...
		case i_data:
			l0 = getnum();
			l1 = getnum();
			initialisation(var_trans[l0], str_trans[l1], getnum());
		...
		case i_var:
			...
			case v_arglist:
				...
				while (getnum() != 0)
				{
					add_argument(ap, type_trans[getnum()]);
					skip();
					skip();
					var_index++;
				}
			...
			case v_array_size:
				l0 = getnum();
				array_size(var_trans[l0], type_trans[getnum()]);
				break;
			...
			case v_static:
				...
				l1 = getnum();
				l2 = getnum();
				var_trans[var_index++] = declare(obj_id(i), str_trans[l0], type_trans[l1], str_trans[l2], getnum());
				break;

			case v_varargs:
				l0 = getnum();
				set_varargs(var_trans[l0], getnum());
				break;
	...
	for (j = 1; j < n; j++)
	{
		...
		case v_static:
			...
			error:
				say_file();
				Fprint(1,", static %s %s, ", diag, p->v_name->sy_name);
				put_file(p->v_file, p->v_line);
				Fprint(1,", never defined\n");
				errors++;
				break;
			...
			case t_ftnreturning:
				...
				for (a = p->v_args; a != NULL; a = a->a_next)
				{
					...
					{
						say_file();
						Fprint(1,": static function %s: ", p->v_name->sy_name);
						put_file(p->v_file, p->v_line);
						Fprint(1,"\n");
	...
	if (p->i_init != NULL)
	{
		...
		if (p->i_argdefn != NULL)
		{
			for (a = p->i_args; a != NULL; a = a->a_next)
			{
				...
				{
					Fprint(1,"function %s: ", p->i_name->sy_name);

...
int pid;
{
	...
	rpp->p_stat = SIDL;
	rpp->p_flag = SLOAD | (rip->p_flag & (SSEXEC|SPAGI|STRC|SPROCTR|SNOCHK));
	rpp->p_uid = rip->p_uid;
	...
	for(n=0; n< NOFILE; n++) {
		...
		register struct fildes *r, *s;

		if((r = u.u_ofile[n]) != NULL) {
			s = f_glue(rpp, r->fr_file);
			s->fr_pofile = r->fr_pofile;
			s->fr_fd = n;		/* used at BETA below */
		}
	}

	rpp->p_lab = labCPX(rip->p_lab);
	rpp->p_ceil = labCPX(rip->p_ceil);
	rpp->p_ceillab = labCP(rip->p_ceillab);
	...
	if (procdup(rpp)) {
		register struct fildes *s;
		
		bzero((caddr_t)u.u_ofile, sizeof(u.u_ofile));
		for(s=rpp->p_ofile;s;s=s->fr_np)
			u.u_ofile[s->fr_fd] = s;	/* BETA */
		return (1);
	}
	(void) spl6();
	rpp->p_stat = SRUN;

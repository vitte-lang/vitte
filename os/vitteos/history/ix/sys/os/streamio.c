...
#include "sys/inode.h"
#include "sys/label.h"		/* needed only for jl_t */
#include "sys/filio.h"
#include "sys/ttyio.h"
#include "sys/conf.h"
#include "sys/proc.h"
...
struct inode *ip;
{
	...
	if (sp = ip->i_sptr) {		/* already streaming? */
		...
		qp->flag |= QBIGB;
		WR(qp)->flag |= QDELIM;
		bzero(sp->source, sizeof(sp->source));
		ip->i_sptr = sp;
		s = spl6();
...
struct inode *ip;
{
	 ...
	 */
	for (;;) {
		if(ip->i_pexflag&PEX_IMPURE) break;
		s = spl6();
...
out:
	if((nc==0) && (ip->i_pexflag&PEX_IMPURE))
		u.u_error = ECONC;
	stexit(ip);
}
...
top:
		if(ip->i_pexflag&PEX_IMPURE) break;
		s = spl6();
		if ((stq->flag&HUNGUP)==0 && stq->wrq->next->flag&QFULL) {
			stq->flag |= WSLEEP;
...
caddr_t arg;
{
	...
	case FIOGSRC:
		if(copyout((caddr_t)stq->source, arg, sizeof(stq->source)))
			u.u_error = EFAULT;
		goto out;
	...
	case FIOSSRC:
		...
		if(copyin(arg, (caddr_t)stq->source, sizeof(stq->source)))
			u.u_error = EFAULT;
		if(loggable(LX))
			seclog(LOG_EXTERN, 90, "CI", ip); 
		goto out;
	...
	/*
	 * hobson's choice: xflush does not reach the device
	 * but flush (which does) is asynchronous and leaves 
	 * goo in the stream
	 */
	case TIOCXFLUSH:
		if (stq->flag & HUNGUP) {
...
register struct file *fp;
{
	...
	bcopy(u.u_logname,((struct kpassfd *)bp->rptr)->logname,
		sizeof(u.u_logname));
	((struct kpassfd *)bp->rptr)->cap = jl_t(u.u_procp->p_lab);
...
caddr_t arg;
{
	...
	i = f_open1(-1, kp->f.fp);		/* f_count = n+2 */
	closef(kp->f.fp);			/* f_count = n+1 */
	if(i == -1) {
		freeb(bp);
		return;
	}
	u.u_r.r_val1 = pfd.fd = i;  /* plain 9vr2 makes val1 = fd */
	pfd.uid = kp->uid;
	pfd.gid = kp->gid;
	pfd.cap = kp->cap;
...
/*
 * total number of blocks in all queues in a stream
 * pipes' blocks are counted in both streams.
 * (secure unix)
 */
sqlen(st)
register struct stdata *st;
{
	register n;
	register struct queue *q;

	n = 0;
	for(q=st->wrq; q; q=q->next) {
		n += q->count;
		n += RD(q)->count;
	}
	return n;

 ...
 *	minor device 4 is public part of kernel memory (read only)(EXTINCT)
...
int mmread(), mmwrite();
daddr_t devnulldev = makedev(3,2);
	...
	case 0:
		while (u.u_count != 0 && u.u_error == 0) {
			...
			*(int *)mmap = v | (PG_V | PG_KR);
			...
			u.u_segflg = SEGUDATA;
			iomove((caddr_t)&vmmap[o], c, B_READ);
		}
		return;
	...
	case 0:
		while (u.u_count != 0 && u.u_error == 0) {
			...
			*(int *)mmap = v | (PG_V | PG_KW);
			...
			c = min(c, (unsigned)(NBPG - ((int)u.u_base&PGOFSET)));
			u.u_segflg = SEGUDATA;
			iomove((caddr_t)&vmmap[o], c, B_WRITE);
		}
		return;

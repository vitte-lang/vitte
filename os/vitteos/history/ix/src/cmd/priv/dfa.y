...
#define CRANK(c) y->delta[c][state]
...
char *os = s;
	...
	if(y->constring) {
		...
		if(debug)
			fprintf(stderr,"dfamatch x(%s,%s)=%d\n",
				y->constring, os, c);
		return c;
	}
	if(y->delta == 0) rere(y);
	...
	state = CRANK(RIGHT);
	if(debug)
		fprintf(stderr,"dfamatch(%s,%s)=%d\n",
			y->pat, os, y->out[state]);
	return y->out[state];
}
...
if(debug) fprintf(stderr,"predfle(%s,%s)\n", x->s, y->s);

	if(x->constring != NULL && y->constring != NULL)
	...
	free(seen);
if(debug) fprintf(stderr,"dfle(%s,%s)=%d\n", x->pat, y->pat, i);
	return i;
}

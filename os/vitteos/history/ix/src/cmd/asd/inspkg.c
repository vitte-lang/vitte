	...
	register FILE *file;
{
	...
	while (--size >= 0) {
		...
		if (putc (c, tf) == EOF) {
			verror ("inspkg: Instructions");
			exit (1);
		}
	}
	if (fclose (tf) == EOF) {
		verror ("inspkg: Instructions fclose");
		exit (1);
	}
	...
	register FILE *file;
{
	...
	while ((c = getc (tf)) != EOF) {
		...
		case 'f':
			...
			case INSTALL:
				 ...
				 *	It is commented out because secure
				 *	system doesn't permit chown of setuid file
				 */
				if (!nflag) {
					time_t timep[2];
					...
					chown (path, uid, gid);
					if(chmod (path, hdr.mode & 07777) < 0) {
						fprintf(stderr,"can't chmod ");
						verror(path);
					}
					setlab(path, &lab);
	...
	if (ferror (tf))
		verror ("inspkg: internal temp file");
...
/* half-hearted job of label preservation */
savlab(path, labp)
char *path;
struct label *labp;
{
	extern char *labtoa();
	struct label temp;
	if(getflab(path, labp) < 0) {
		if(errno != ENOENT) {
			fprintf(stderr, "inspkg: can't getflab ");
			verror(path);
		}
		getplab(labp, &temp);
		labp->lb_t = labp->lb_u = labp->lb_fix = 0;
	}
	if(labp->lb_t == 0 && labp->lb_u == 0)
		return;
	temp = *labp;
	temp.lb_t = temp.lb_u = 0;
	if(setflab(path, &temp) < 0) {
		fprintf(stderr, "inspkg: can't unset privilege ");
		verror(path);
	}
}
...
struct label *labp;
{
	if(setflab(path, labp) < 0) {
		fprintf(stderr, "inspkg: can't reset label ");
		fprintf(stderr, "%s", labtoa(labp));
		verror(path);
	}
}

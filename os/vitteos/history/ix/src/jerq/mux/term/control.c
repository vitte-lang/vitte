...
#define MAXNPROC 16
struct Pchannel pconvs[MAXNPROC];
...
tborder(l, r, i, t, c)	/* no flashing! */
	...
	Code c;
{
	Utexture(l, Rect(r.origin.x, r.origin.y, r.corner.x, r.origin.y+i), t, c);
	Utexture(l, Rect(r.origin.x, r.corner.y-i, r.corner.x, r.corner.y), t, c);
	Utexture(l, Rect(r.origin.x, r.origin.y+i, r.origin.x+i, r.corner.y-i), t, c);
	Utexture(l, Rect(r.corner.x-i, r.origin.y+i, r.corner.x, r.corner.y-i), t, c);
}
	...
	register struct Proc *p;
{
	...
	else if(pextab[p-proctab])
		tborder(p->layer, p->layer->rect, INSET, &pexchex, F_XOR);
}
cansend(channel)
{
	register i;

...
register struct inode *ip;
{
	...
	if(n > 0) {	/* why not iomove? */
		if(u.u_segflg != SEGSYS) {
			if(copyout(rp + RDISIZE, u.u_base, n))
				u.u_error = EFAULT;
		}
		else
			bcopy(rp + RDISIZE, u.u_base, n);
	}
	u.u_offset = Lladd(u.u_offset, frnetlong(rp, RDI_USED));
	u.u_r.r_val1 = n;
out:
	putnbbuf(rp);
}
...
struct inode *dip;
{
	...
	if (dip->i_fstyp == fstyp && major(dip->i_dev) == major(flag)) {
		u.u_error = EBUSY;
		return;
	}
	/*
	...
	dev = makedev(major(flag), frnetchar(pp, RNM_DEV));
	if ((rip = iget(&pi, dev, frnetlong(pp, RNM_INO))) == NULL)
		return;
	if (rip->i_count != 1) {
		iput(rip);
		u.u_error = EBUSY;
...
/*
 * unmount a netb--
 * called from above, or from nbsend when there's an error

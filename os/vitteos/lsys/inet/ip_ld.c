	...
	while(bp = getq(q)){
		...
		case M_IOCTL:
			...
			case IPIONET:
				...
				ifp->broadcast = ifp->that|~ifp->mask;
				ip_ifinsert(ifp);
				ifp->flags &= (~IFF_HOST);
				bp->type = M_IOCACK;
				qreply(q, bp);
	...
	register u_long addr;
{
	...
	for(i=0; i < Ipcnt; i++){
		...
		if(ifp->flags & IFF_UP) {
			...
			/* standard IP broadcast address */
			if(addr == ifp->broadcast)
				return(ifp);

			/* old style BSD IP broadcast address */
			...
			/* finally, try unsubnetted broadcast addresses */
			mask = IN_CLASS_NMASK(ifp->thishost);
			if((mask&ifp->thishost | ~mask) == addr)
				return(ifp);

			/* finally, try unsubnetted broadcast addresses */
			if((mask&ifp->thishost) == addr)
				return(ifp);
		}
	}
	return(0);

...
# If runner uses an incompatible option and `FORCE_CI_C` is not defined,
# switch to in-tree c.
if [ "$FORCE_CI_C" == "" ]; then
    echo 'debug: `DISABLE_CI_C_IF_INCOMPATIBLE` configured.'
    DISABLE_CI_C_IF_INCOMPATIBLE=1
fi

_CONFIGURE_ARGS="$_CONFIGURE_ARGS --set build.print-step-timings --enable-verbose-tests"
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --set build.metrics"
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-verbose-configure"
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-sccache"
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --disable-manage-submodules"
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-locked-deps"
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-cargo-native-static"
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --set .codegen-units-std=1"
# -lang/promote-release will recompress CI artifacts, and while we care
...
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --set dist.compression-profile=balanced"
...
# https://github.com/-lang//issues/108227.
if isKnownToBeMingwBuild; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set llvm.link-jobs=1"
fi

# Only produce xz tarballs on CI. gz tarballs will be generated by the release
# process by recompressing the existing xz ones. This decreases the storage
# space required for CI artifacts.
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --dist-compression-formats=xz"

# Enable the `c` feature for compiler_builtins, but only when the `compiler-rt` source is available
# (to avoid spending a lot of time cloning llvm)
if [ "$EXTERNAL_LLVM" = "" ]; then
  # Enable building & shipping lld
  _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set .lld=true"

  _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set build.optimized-compiler-builtins"
...
if [ "$DIST_SRC" = "" ]; then
  _CONFIGURE_ARGS="$_CONFIGURE_ARGS --disable-dist-src"
fi
...
export _RELEASE_CHANNEL=$(releaseChannel)
_CONFIGURE_ARGS="$_CONFIGURE_ARGS --release-channel=$_RELEASE_CHANNEL"

if [ "$DEPLOY$DEPLOY_ALT" = "1" ]; then
  if [[ "$CI_JOB_NAME" == *ohos* ]]; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --disable-llvm-static-stdcpp"
  else
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-llvm-static-stdcpp"
  fi
  _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set .remap-debuginfo"

  if [ "$DEPLOY_ALT" != "" ] && isLinux; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --debuginfo-level=2"
  else
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --debuginfo-level-std=1"
  fi

  if [ "$NO_LLVM_ASSERTIONS" = "1" ]; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --disable-llvm-assertions"
  elif [ "$DEPLOY_ALT" != "" ]; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-llvm-assertions"
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set .verify-llvm-ir"
  fi

  CODEGEN_BACKENDS="${CODEGEN_BACKENDS:-llvm}"
  _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set .codegen-backends=$CODEGEN_BACKENDS"

  # Unless explicitly disabled, we want c debug assertions on the -alt builds
  if [ "$DEPLOY_ALT" != "" ] && [ "$NO_DEBUG_ASSERTIONS" = "" ]; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-debug-assertions"
  fi
else
  ...
  if [ "$NO_OVERFLOW_CHECKS" = "" ]; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-overflow-checks"
  fi
  ...
  if [ "$NO_DOWNLOAD_CI_LLVM" = "" ]; then
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set llvm.download-ci-llvm=if-unchanged"
  else
    # CI c requires CI LLVM to be enabled (see https://github.com/-lang//issues/123586).
    NO_DOWNLOAD_CI_C=1
    # When building for CI we want to use the static C++ Standard library
    # included with LLVM, since a dynamic libstdcpp may not be available.
    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set llvm.static-libstdcpp"
  fi

  # Download GCC from CI on test builders
  _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set gcc.download-ci-gcc=true"

  # download-c seems to be broken on CI after the stage0 redesign
  # Disable it until these issues are debugged and resolved
#  if [ "$NO_DOWNLOAD_CI_C" = "" ]; then
#    _CONFIGURE_ARGS="$_CONFIGURE_ARGS --set .download-c=if-unchanged"
#  fi
fi

if [ "$ENABLE_GCC_CODEGEN" = "1" ]; then
  # If `ENABLE_GCC_CODEGEN` is set and not empty, we add the `--enable-new-symbol-mangling`
  # argument to `_CONFIGURE_ARGS` and set the `GCC_EXEC_PREFIX` environment variable.
  # `cg_gcc` doesn't support the legacy mangling so we need to enforce the new one
  # if we run `cg_gcc` tests.
  _CONFIGURE_ARGS="$_CONFIGURE_ARGS --enable-new-symbol-mangling"
fi
...
datecheck() {
  ...
  date
  echo -n "  network time: "
  curl -fs --head http://ci-caches.-lang.org | grep ^Date: \
      | sed 's/Date: //g' || true

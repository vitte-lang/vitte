...
With the ongoing growth of the  language and with that of the whole
...
Additionally, according to the [ Roadmap 2021], clear processes should be
defined by every team and unified across teams. This Roadmap is the first step
towards this.

[ Roadmap 2021]: https://github.com/-lang/rfcs/pull/3037
...
any output. This is especially problematic since `-analyzer` is on the rise,
...
- [#4612](https://github.com/-lang/-clippy/issues/4612)
...
- [#3164](https://github.com/-lang/-clippy/issues/3164)
- [cargo#5034](https://github.com/-lang/cargo/issues/5034)
- [IRLO](https://internals.-lang.org/t/proposal-cargo-lint-configuration/9135/8)
...
lints are less conservative than `c` lints, which won't change in the
future.

- [#5537](https://github.com/-lang/-clippy/issues/5537)
- [#6366](https://github.com/-lang/-clippy/issues/6366)
...
nightly  nowadays makes it more probable that a lint with many FPs hits
...
- [#6429](https://github.com/-lang/-clippy/issues/6429)
...
According to the [ Roadmap 2021], a document specifying what it means to be
...
before  syncs might make sense.

#### Triaging

To get a handle on the influx of open issues, a process for triaging issues and
PRs should be developed. Officially, Clippy follows the  triage process, but
...
- [#6429 (comment)](https://github.com/-lang/-clippy/issues/6429#issuecomment-741056379)
- [#6429 (comment)](https://github.com/-lang/-clippy/issues/6429#issuecomment-741153345)
...
- [#5394](https://github.com/-lang/-clippy/issues/5394)
...
Recently Clippy was integrated with `git subtree` into the `-lang/`
...
1. Use the same `fmt` version and configuration as `c`.
2. Make `cargo dev` work in the  repo, just as it works in the Clippy repo.
   E.g. `cargo dev bless` or `cargo dev update_lints`. And even add more things
   to it that might be useful for the  repo, e.g. `cargo dev deprecate`.
...
's roadmap process was established by [RFC 1728] in 2016. Since then every

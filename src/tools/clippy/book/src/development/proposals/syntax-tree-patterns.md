...
- RFC PR: [#3875](https://github.com/-lang/-clippy/pull/3875)
...
A pattern can also contain  literals. These literals match themselves.
...
[here](https://github.com/fkohlgrueber/-clippy-pattern/blob/039b07ecccaf96d6aa7504f5126720d2c9cceddd/clippy_lints/src/collapsible_if.rs#L88-L163).
...
pattern. Matching a *PatternTree* against an actual syntax tree (e.g.  ast /
...
It's a data structure similar to 's AST / HIR, but with the following
differences:
...
implementations ( ast / hir, syn, ...). When looking at the different
...
is similar to actual  syntax (probably like the `quote!` macro). For
...
Extending  syntax (which is quite complex by itself) with additional syntax
...
Naming subpatterns in a -like syntax is difficult because a lot of AST nodes
...
In general,  syntax contains a lot of code structure implicitly. This
...
at least as complex as the  parser itself. Additionally, future changes in
the  syntax might be incompatible with such a syntax.
...
by a tool that, given a  program, generates a pattern that matches only this
program (similar to the Clippy author lint).

For some simple cases (like the first example above), it might be possible to
successfully mix  and pattern syntax. This space could be further explored
...
#### Implement rest of  Syntax

The current project only implements a small part of the  syntax. In the
future, this should incrementally be extended to more syntax to allow
implementing more lints. Implementing more of the  syntax requires extending
...
Another improvement could be to create a tool that, given some valid 
...
generated for a couple of  code examples and use that information to write a

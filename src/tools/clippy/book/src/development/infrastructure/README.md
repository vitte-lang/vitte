...
In order to deploy Clippy over `up`, some infrastructure is necessary. This
chapter describes the different parts of the Clippy infrastructure that need to
be maintained to make this possible.

The most important part is the sync between the `-lang/` repository and
the Clippy repository that takes place every two weeks. This process is
described in the [Syncing changes between Clippy and `-lang/`](sync.md)
section.

A new Clippy release is done together with every  release, so every six

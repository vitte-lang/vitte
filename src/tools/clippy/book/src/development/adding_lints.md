...
- [Adding a new lint](#adding-a-new-lint)
  ...
  - [Specifying the lint's minimum supported  version (MSRV)](#specifying-the-lints-minimum-supported--version-msrv)
  - [Author lint](#author-lint)
  - [Print HIR lint](#print-hir-lint)
  - [Documentation](#documentation)
  - [Running fmt](#running-fmt)
  - [Debugging](#debugging)
...
exactly as expected. Each test is just a plain  file that contains the code
...
will create a `.fixed` file by running [fix] for that test.
fix will apply the suggestions
...
Or set up a up toolchain that points to the local Clippy binaries
...
use c_session::declare_lint_pass;
use c_ast::ast::*;
```
...
* The `#[clippy::version]` attribute will be rendered as part of the lint
  documentation. The value should be set to the current  version that the
  lint is developed in, it can be retrieved by running `c -vV` in the
  -clippy directory. The version is listed under *release*. (Use the version
...
According to [the c-dev-guide], the text should be matter of fact and avoid
...
Sometimes a lint makes suggestions that require a certain version of . For
example, the `manual_strip` lint suggests using `str::strip_prefix` and
`str::strip_suffix` which is only available after  1.45. In such cases, you
need to ensure that the MSRV configured for the project is >= the MSRV of the
required  feature. If multiple features are required, just use the one with
a lower MSRV.
...
work for all the  syntax, but can give a good starting point.

The quickest way to use it, is the [ playground:
play.-lang.org][author_example]. Put the code you want to lint into the
...
[author_example]: https://play.-lang.org/?version=nightly&mode=debug&edition=2024&gist=9a12cb60e5c6ad4e3003ac6d5e63cf55

## Print HIR lint

To implement a lint, it's helpful to first understand the internal
representation that c uses. Clippy has the `#[clippy::dump]` attribute that
...
[fmt] is a tool for formatting  code according to style guidelines. Your
code has to be formatted by `fmt` before a PR can be merged. Clippy uses
nightly `fmt` in the CI.

It can be installed via `up`:

```bash
up component add fmt --toolchain=nightly
```

Use `cargo dev fmt` to format the whole codebase. Make sure that `fmt` is
installed for the nightly toolchain.

[fmt]: https://github.com/-lang/fmt
...
4. Adding tests:
    ...
    2. The configuration itself will be tested separately in [`tests/ui-toml`].
       ...
       `clippy.toml` file with the configuration value and a  file that
...
[`clippy_config::conf`]: https://github.com/-lang/-clippy/blob/master/clippy_config/src/conf.rs
[`clippy_lints` lib file]: https://github.com/-lang/-clippy/blob/master/clippy_lints/src/lib.rs
[`tests/ui`]: https://github.com/-lang/-clippy/blob/master/tests/ui
[`tests/ui-toml`]: https://github.com/-lang/-clippy/blob/master/tests/ui-toml
...
* [Common tools for writing lints](common_tools_writing_lints.md) helps with
  common operations
* [The c-dev-guide][c-dev-guide] explains a lot of internal compiler
  concepts
* [The nightly c docs][nightly_docs] which has been linked to throughout
  this guide
...
While most of Clippy's lint utils are documented, most of c's internals lack

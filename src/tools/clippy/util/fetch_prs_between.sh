...
# exclude the non-user facing changes like 'up' PRs, typo fixes, etc.
...
for pr in $(git log --oneline --merges --first-parent "$1...$2"); do
  ...
  echo "URL: https://github.com/-lang/-clippy/pull/$id"
  echo "Markdown URL: [#$id](https://github.com/-lang/-clippy/pull/$id)"
  echo "$message"
  echo "---------------------------------------------------------"
  echo
done

newest_merged_at="$(gh pr view -R -lang/-clippy --json mergedAt $newest_pr -q .mergedAt)"
oldest_merged_at="$(gh pr view -R -lang/-clippy --json mergedAt $oldest_pr -q .mergedAt)"

query="merged:$oldest_merged_at..$newest_merged_at base:master"
encoded_query="$(echo $query | sed 's/ /+/g; s/:/%3A/g')"

pr_link="https://github.com/-lang/-clippy/pulls?q=$encoded_query"
count="$(gh api -X GET search/issues -f "q=$query repo:-lang/-clippy" -q .total_count)"

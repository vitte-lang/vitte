...
Register-ArgumentCompleter -Native -CommandName 'x.py' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'x.py' {
            ...
            [CompletionResult]::new('--c-error-format', '--c-error-format', [CompletionResultType]::ParameterName, 'c error format')
            [CompletionResult]::new('--on-fail', '--on-fail', [CompletionResultType]::ParameterName, 'command to run on failure')
            [CompletionResult]::new('--stage', '--stage', [CompletionResultType]::ParameterName, 'stage to build (indicates compiler to use/test, e.g., stage 0 uses the bootstrap compiler, stage 1 the stage 0 c artifacts, etc.)')
            ...
            [CompletionResult]::new('--src', '--src', [CompletionResultType]::ParameterName, 'path to the root of the  checkout')
            ...
            [CompletionResult]::new('--color', '--color', [CompletionResultType]::ParameterName, 'whether to use color in cargo and c output')
            [CompletionResult]::new('---profile-generate', '---profile-generate', [CompletionResultType]::ParameterName, 'generate PGO profile with c build')
            [CompletionResult]::new('---profile-use', '---profile-use', [CompletionResultType]::ParameterName, 'use PGO profile for c build')
            ...
            [CompletionResult]::new('--compile-time-deps', '--compile-time-deps', [CompletionResultType]::ParameterName, 'only build proc-macros and build scripts (for -analyzer)')
            ...
            [CompletionResult]::new('--llvm-profile-generate', '--llvm-profile-generate', [CompletionResultType]::ParameterName, 'generate PGO profile with llvm built for c')
            ...
            [CompletionResult]::new('--skip-std-check-if-no-download-c', '--skip-std-check-if-no-download-c', [CompletionResultType]::ParameterName, 'Skip checking the standard library if `.download-c` isn''t available. This is mostly for RA as building the stage1 compiler to check the library tree on each code change might be too much for some computers')
            ...
            [CompletionResult]::new('clippy', 'clippy', [CompletionResultType]::ParameterValue, 'Run Clippy (uses up/cargo-installed clippy binary)')
            [CompletionResult]::new('fix', 'fix', [CompletionResultType]::ParameterValue, 'Run cargo fix')
            [CompletionResult]::new('fmt', 'fmt', [CompletionResultType]::ParameterValue, 'Run fmt')
            ...
            [CompletionResult]::new('perf', 'perf', [CompletionResultType]::ParameterValue, 'Perform profiling and benchmarking of the compiler using `c-perf`')
            break
        }
        'x.py;build' {
            [CompletionResult]::new('--config', '--config', [CompletionResultType]::ParameterName, 'TOML configuration file for build')
        ...
        'x.py;test' {
            [CompletionResult]::new('--test-args', '--test-args', [CompletionResultType]::ParameterName, 'extra arguments to be passed for the test tool being used (e.g. libtest, compiletest or doc)')
            [CompletionResult]::new('--compiletest-c-args', '--compiletest-c-args', [CompletionResultType]::ParameterName, 'extra options to pass the compiler when running compiletest tests')
            ...
            [CompletionResult]::new('--fix-coverage', '--fix-coverage', [CompletionResultType]::ParameterName, 'enable this to generate a fix coverage file, which is saved in `/<build_base>/fix_missing_coverage.txt`')

...
[#54192]: https://github.com/-lang//issues/54192

------------------------

The c flag `-Z emit-stack-sizes` makes LLVM emit stack size metadata.
...
``` console
$ c -C opt-level=3 --emit=obj foo.rs
...
$ c -C opt-level=3 --emit=obj -Z emit-stack-sizes foo.rs
...
The linker script must be passed to the linker using a c flag like `-C
link-arg`.
...
``` console
$ FLAGS="-Z emit-stack-sizes" cargo build --release

$ size -A target/release/hello | grep stack_sizes || echo section was not found
section was not found

$ FLAGS="-Z emit-stack-sizes" cargo c --release -- \

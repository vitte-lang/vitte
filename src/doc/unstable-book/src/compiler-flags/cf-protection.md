...
The tracking issue for this feature is: [#93754](https://github.com/-lang//issues/93754).
...
together must have the flags set for the compiled output to be CET-enabled. Currently, 's
...
```sh
$ FLAGS="-Z cf-protection=full" cargo +nightly build -Z build-std --target x86_64-unknown-linux-gnu
```
...
```sh
$ C_LOG=c_codegen_ssa::back::link=info c-custom -v -Z cf-protection=full -C link-arg="-Wl,-z,cet-report=warning" -o example example.rs
...
/usr/bin/ld: /.../build/x86_64-unknown-linux-gnu/stage1/lib/lib/x86_64-unknown-linux-gnu/lib/libstd-d73f7266be14cb8b.rlib(std-d73f7266be14cb8b.std.f7443020-cgu.12.rcgu.o): warning: missing IBT and SHSTK properties
```

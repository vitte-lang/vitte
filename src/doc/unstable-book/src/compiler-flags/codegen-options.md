...
All of these options are passed to `c` via the `-C` flag, short for "codegen". The flags are
...
- `ptx`: use [`-ptx-linker`](https://github.com/denzp/-ptx-linker)
  for Nvidia NVPTX GPGPU support.
- `bpf`: use [`bpf-linker`](https://github.com/alessandrod/bpf-linker) for eBPF support.
- `llbc`: for linking in llvm bitcode. Install the preview up components`llvm-bitcode-linker`
...
This flag generally controls whether the linker will use libraries and objects shipped with 
...
It refers to the LLD linker, built from the same LLVM revision used by c (named `-lld` to
avoid naming conflicts), that is distributed via `up` with the compiler (and is used by default
...
toolchain's `-lld` as the linker instead of the system's lld with `-Clinker-features=+lld` only.

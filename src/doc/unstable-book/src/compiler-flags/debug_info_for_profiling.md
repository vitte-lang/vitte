...
Unlike PGO (controlled by the `c` flags `-Cprofile-generate` and
...
Given a  file `main.rs`, we can produce an optimized binary as follows:

```shell
c -O -Zdebug-info-for-profiling main.rs -o main
perf record -b ./main
create_llvm_prof --binary=main --out=code.prof
c -O -Zprofile-sample-use=code.prof main.rs -o main2
```
...
used by `c` to guide optimizations in producing the binary `main2`.

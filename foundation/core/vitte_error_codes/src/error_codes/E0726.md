...
the  compiler to know, on usage, the lifespan of the type. When the
lifetime is not explicitly mentioned and the  Compiler cannot determine
...
let content = Content { title: "", body: "is great!" };
let future = create(content);
block_on(future);
```

Specify desired lifetime of parameter `content` or indicate the anonymous
lifetime like `content: Content<'_>`. The anonymous lifetime tells the 
...
[lifetime-elision]: https://doc.-lang.org/book/ch10-03-lifetime-syntax.html#lifetime-elision
[lifetimes]: https://doc.-lang.org/-by-example/scope/lifetime.html

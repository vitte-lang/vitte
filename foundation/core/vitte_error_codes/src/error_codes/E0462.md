...
library only intended for linking with non- applications (C programs). Note
 ...
 * Using [Cargo](../cargo/index.html), the  package manager, automatically
   fixing this issue.
 * Recompiling the crate as a `rlib` or `dylib`; formats suitable for 
   linking.

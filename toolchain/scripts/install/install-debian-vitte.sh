#!/usr/bin/env bash
# ============================================================
# vitte — Debian/Ubuntu installer
# Location: toolchain/scripts/install/install-debian-vitte.sh
# ============================================================
...
EXPECTED_PKG_VERSION="${EXPECTED_PKG_VERSION:-$(tr -d ' \r\n' < "$PKG_VERSION_FILE" 2>/dev/null || echo unknown)}"
VERBOSE="${VERBOSE:-0}"
DEPS_FILE="${DEPS_FILE:-$ROOT_DIR/toolchain/scripts/install/debian-deps.sh}"

log() { printf "[install-debian] %s\n" "$*"; }
die() { printf "[install-debian][error] %s\n" "$*" >&2; exit 1; }

...
static int forward_to_vitte(const std::string& vitte_path, int argc, char** argv) {
    ...
    if (input.empty()) {
        std::cerr << "vittec1: missing input file\n";
        return 1;
    }
...
int main(int argc, char** argv) {
    if (argc >= 2 && std::string(argv[1]) == "--version") {
        std::cout << "vittec1 stage1 wrapper\n";
        return 0;
    }

    if (argc < 2) {
        std::cerr << "vittec1: expected command (parse/build)\n";
        return 1;
    }

    std::string vitte_path = find_vitte(argv[0]);
    if (vitte_path.empty()) {
        std::cerr << "vittec1: cannot find bin/vitte. Build the project first.\n";
        return 1;
    }
